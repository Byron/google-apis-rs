...
use google_discovery1::{api, yup_oauth2, Error};
...
{
    async fn _apis_get_rest(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.apis().get_rest(
            opt.value_of("api").unwrap_or(""),
            opt.value_of("version").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            ...
            .iter()
        {
            ...
            match key {
                "preferred" => {
                    call = call.preferred(
                        value
                            .map(|v| arg_from_str(v, err, "preferred", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "name" => {
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("apis", Some(opt)) => match opt.subcommand() {
                ("get-rest", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("apis".to_string()));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "discovery1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/discovery1", config_dir))
        .build()
        .await
        .unwrap();
