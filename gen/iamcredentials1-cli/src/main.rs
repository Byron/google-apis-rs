...
extern crate mime;
extern crate strsim;
extern crate google_iamcredentials1 as api;
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::IAMCredentials<hyper::Client, Authenticator<DefaultAuthenticatorDelegate, JsonTokenStorage, hyper::Client>>,
    gp: Vec<&'static str>,
...
impl<'n> Engine<'n> {
    fn _projects_service_accounts_generate_access_token(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    fn _projects_service_accounts_generate_id_token(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
            ...
            ("projects", Some(opt)) => {
                match opt.subcommand() {
                    ("service-accounts-generate-access-token", Some(opt)) => {
                        call_result = self._projects_service_accounts_generate_access_token(opt, dry_run, &mut err);
                    },
    ...
    fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match cmn::application_secret_from_directory(&config_dir, "iamcredentials1-secret.json",
