...
use google_sts1::{api, Error, oauth2};
    ...
    S::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    async fn _methods_token(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    "requested-token-type" => Some(("requestedTokenType", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Pod })),
                    "scope" => Some(("scope", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Pod })),
                    "subject-token" => Some(("subjectToken", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Pod })),
                    "subject-token-type" => Some(("subjectTokenType", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Pod })),
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["audience", "grant-type", "options", "requested-token-type", "scope", "subject-token", "subject-token-type"]);
                        err.issues.push(CLIError::Field(FieldError::Unknown(temp_cursor.to_string(), suggestion, value.map(|v| v.to_string()))));
                        None
                    }
                };
        ...
        let mut request: api::GoogleIdentityStsV1ExchangeTokenRequest = json::value::from_value(object).unwrap();
    ...
    async fn new(opt: ArgMatches<'n>, connector: S) -> Result<Engine<'n, S>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "sts1-secret.json",
        ...
        let auth = oauth2::InstalledFlowAuthenticator::with_client(
            ...
            client.clone(),
        ).persist_tokens_to_disk(format!("{}/sts1", config_dir)).build().await.unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ("methods", "methods: 'introspect' and 'token'", vec![
            ("introspect",
                    Some(r##"Gets information about a Google OAuth 2.0 access token issued by the Google Cloud [Security Token Service API](https://cloud.google.com/iam/docs/reference/sts/rest)."##),
            ...
            ("token",
                    Some(r##"Exchanges a credential for a Google OAuth 2.0 access token. The token asserts an external identity within an identity pool, or it applies a Credential Access Boundary to a Google access token. When you call this method, do not send the `Authorization` HTTP header in the request. This method does not require the `Authorization` header, and using the header can cause the request to fail."##),
