...
use google_gan1_beta1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _advertisers_get(
        ...
        let mut call = self.hub.advertisers().get(
            opt.value_of("role").unwrap_or(""),
            opt.value_of("role-id").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
    ...
    async fn _advertisers_list(
            ...
            .iter()
        {
            ...
            match key {
                ...
                "min-seven-day-epc" => {
                    call = call.min_seven_day_epc(
                        value
                            .map(|v| arg_from_str(v, err, "min-seven-day-epc", "double"))
                            .unwrap_or(0.0),
                    );
                }
                "min-payout-rank" => {
                    call = call.min_payout_rank(
                        value
                            .map(|v| arg_from_str(v, err, "min-payout-rank", "int32"))
                            .unwrap_or(-0),
                    );
                }
                "min-ninety-day-epc" => {
                    call = call.min_ninety_day_epc(
                        value
                            .map(|v| arg_from_str(v, err, "min-ninety-day-epc", "double"))
                            .unwrap_or(0.0),
                    );
                }
                "max-results" => {
    ...
    async fn _cc_offers_list(
        ...
        let mut call = self
            .hub
            .cc_offers()
            .list(opt.value_of("publisher").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _events_list(
        ...
        let mut call = self.hub.events().list(
            opt.value_of("role").unwrap_or(""),
            opt.value_of("role-id").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _links_get(
        ...
        let link_id: i64 = arg_from_str(
            &opt.value_of("link-id").unwrap_or(""),
            err,
            "<link-id>",
            "int64",
        );
        let mut call = self.hub.links().get(
            opt.value_of("role").unwrap_or(""),
            opt.value_of("role-id").unwrap_or(""),
            link_id,
        );
    ...
    async fn _links_insert(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "epc-ninety-day-average.amount" => Some((
                    "epcNinetyDayAverage.amount",
                    JsonTypeInfo {
                        jtype: JsonType::Float,
                        ctype: ComplexType::Pod,
                    },
                )),
                "epc-ninety-day-average.currency-code" => Some((
                    "epcNinetyDayAverage.currencyCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "epc-seven-day-average.amount" => Some((
                    "epcSevenDayAverage.amount",
                    JsonTypeInfo {
                        jtype: JsonType::Float,
                        ctype: ComplexType::Pod,
                    },
                )),
                "epc-seven-day-average.currency-code" => Some((
                    "epcSevenDayAverage.currencyCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                ...
                "special-offers.free-gift" => Some((
                    "specialOffers.freeGift",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "special-offers.free-shipping" => Some((
                ...
                "special-offers.free-shipping-min.amount" => Some((
                    "specialOffers.freeShippingMin.amount",
                    JsonTypeInfo {
                        jtype: JsonType::Float,
                        ctype: ComplexType::Pod,
                    },
                )),
                "special-offers.free-shipping-min.currency-code" => Some((
                    "specialOffers.freeShippingMin.currencyCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "special-offers.percent-off" => Some((
                    "specialOffers.percentOff",
                    JsonTypeInfo {
                        jtype: JsonType::Float,
                        ctype: ComplexType::Pod,
                    },
                )),
                "special-offers.percent-off-min.amount" => Some((
                    "specialOffers.percentOffMin.amount",
                    JsonTypeInfo {
                        jtype: JsonType::Float,
                        ctype: ComplexType::Pod,
                    },
                )),
                "special-offers.percent-off-min.currency-code" => Some((
                    "specialOffers.percentOffMin.currencyCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "special-offers.price-cut.amount" => Some((
                    "specialOffers.priceCut.amount",
                    JsonTypeInfo {
                        jtype: JsonType::Float,
                        ctype: ComplexType::Pod,
                    },
                )),
                "special-offers.price-cut.currency-code" => Some((
                    "specialOffers.priceCut.currencyCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "special-offers.price-cut-min.amount" => Some((
                    "specialOffers.priceCutMin.amount",
                    JsonTypeInfo {
                        jtype: JsonType::Float,
                        ctype: ComplexType::Pod,
                    },
                )),
                "special-offers.price-cut-min.currency-code" => Some((
                    "specialOffers.priceCutMin.currencyCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "special-offers.promotion-codes" => Some((
                    "specialOffers.promotionCodes",
                    JsonTypeInfo {
                        jtype: JsonType::String,
        ...
        let mut request: api::Link = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.links().insert(
            request,
            opt.value_of("role").unwrap_or(""),
            opt.value_of("role-id").unwrap_or(""),
        );
    ...
    async fn _links_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.links().list(
            opt.value_of("role").unwrap_or(""),
            opt.value_of("role-id").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _publishers_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.publishers().get(
            opt.value_of("role").unwrap_or(""),
            opt.value_of("role-id").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _publishers_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.publishers().list(
            opt.value_of("role").unwrap_or(""),
            opt.value_of("role-id").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _reports_get(
        ...
        let mut call = self.hub.reports().get(
            opt.value_of("role").unwrap_or(""),
            opt.value_of("role-id").unwrap_or(""),
            opt.value_of("report-type").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            ...
            .iter()
        {
            ...
            match key {
                ...
                "calculate-totals" => {
                    call = call.calculate_totals(
                        value
                            .map(|v| arg_from_str(v, err, "calculate-totals", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "advertiser-id" => {
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("advertisers", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("advertisers".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("cc-offers", Some(opt)) => match opt.subcommand() {
                ("list", Some(opt)) => {
                    call_result = self._cc_offers_list(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("cc-offers".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ...
            ("links", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("links".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("publishers", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("publishers".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "gan1-beta1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/gan1-beta1", config_dir))
        .build()
        .await
        .unwrap();
