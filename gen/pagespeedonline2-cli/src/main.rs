...
use google_pagespeedonline2::{api, Error};
...
impl<'n> Engine<'n> {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           ...
                                                                           v.extend(["filter-third-party-resources", "screenshot", "locale", "rule", "strategy"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "pagespeedonline2-secret.json",
        ...
        ).persist_tokens_to_disk(format!("{}/pagespeedonline2", config_dir)).build().await.unwrap();
    ...
    let arg_data = [
        ("pagespeedapi", "methods: 'runpagespeed'", vec![
            ("runpagespeed",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_pagespeedonline2_cli/pagespeedapi_runpagespeed",
                  vec![
                    (Some(r##"url"##),
                     None,
