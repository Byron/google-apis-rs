...
extern crate mime;
extern crate strsim;
extern crate google_driveactivity2 as api;
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::DriveActivityHub<hyper::Client, Authenticator<DefaultAuthenticatorDelegate, JsonTokenStorage, hyper::Client>>,
    gp: Vec<&'static str>,
...
impl<'n> Engine<'n> {
    fn _activity_query(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        match self.opt.subcommand() {
            ("activity", Some(opt)) => {
                match opt.subcommand() {
                    ("query", Some(opt)) => {
                        call_result = self._activity_query(opt, dry_run, &mut err);
                    },
                    _ => {
    ...
    fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match cmn::application_secret_from_directory(&config_dir, "driveactivity2-secret.json",
