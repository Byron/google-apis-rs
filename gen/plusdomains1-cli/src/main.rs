...
use google_plusdomains1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _activities_get(
        ...
        let mut call = self
            .hub
            .activities()
            .get(opt.value_of("activity-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _comments_get(
        ...
        let mut call = self
            .hub
            .comments()
            .get(opt.value_of("comment-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _comments_list(
        ...
        let mut call = self
            .hub
            .comments()
            .list(opt.value_of("activity-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["max-results", "page-token", "sort-order"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
    ...
    async fn _media_insert(
        ...
        let mut request: api::Media = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.media().insert(
            request,
            opt.value_of("user-id").unwrap_or(""),
            opt.value_of("collection").unwrap_or(""),
        );
    ...
    async fn _people_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.people().list(
            opt.value_of("user-id").unwrap_or(""),
            opt.value_of("collection").unwrap_or(""),
        );
        for parg in opt
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["max-results", "order-by", "page-token"].iter().map(|v| *v),
                                );
                                v
                            }));
                    }
    ...
    async fn _people_list_by_activity(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.people().list_by_activity(
            opt.value_of("activity-id").unwrap_or(""),
            opt.value_of("collection").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("activities", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._activities_get(opt, dry_run, &mut err).await;
                }
                ("list", Some(opt)) => {
            ...
            ("audiences", Some(opt)) => match opt.subcommand() {
                ("list", Some(opt)) => {
                    call_result = self._audiences_list(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("audiences".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("circles", Some(opt)) => match opt.subcommand() {
                ("list", Some(opt)) => {
                    call_result = self._circles_list(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("circles".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ...
            ("media", Some(opt)) => match opt.subcommand() {
                ("insert", Some(opt)) => {
                    call_result = self._media_insert(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
            ...
            ("people", Some(opt)) => match opt.subcommand() {
                ...
                ("list", Some(opt)) => {
                    call_result = self._people_list(opt, dry_run, &mut err).await;
                }
                ("list-by-activity", Some(opt)) => {
                    call_result = self._people_list_by_activity(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "plusdomains1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/plusdomains1", config_dir))
        .build()
        .await
        .unwrap();
