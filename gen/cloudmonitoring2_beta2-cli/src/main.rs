...
impl Engine {
    ...
    fn _metric_descriptors_delete(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
    ...
    fn _doit(&self, dry_run: bool) -> (Option<api::Error>, Option<InvalidOptionsError>) {
        ...
        if self.opt.cmd_metric_descriptors {
            if self.opt.cmd_create {
                call_result = self._metric_descriptors_create(dry_run, &mut err);
            } else if self.opt.cmd_delete {
                call_result = self._metric_descriptors_delete(dry_run, &mut err);
            } else if self.opt.cmd_list {
                call_result = self._metric_descriptors_list(dry_run, &mut err);
            } else {
                unreachable!();
            }
            ...
            if self.opt.cmd_list {
                ...
                call_result = self._timeseries_write(dry_run, &mut err);
            } else {
                unreachable!();
            }
