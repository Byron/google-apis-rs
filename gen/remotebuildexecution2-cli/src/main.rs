...
use google_remotebuildexecution2::{api, yup_oauth2, Error};
...
struct Engine<'n, C> {
    opt: ArgMatches<'n>,
    hub: api::RemoteBuildExecution<C>,
    gp: Vec<&'static str>,
    ...
    C: apis_common::Connector,
{
    async fn _action_results_get(
            ...
            .iter()
        {
            ...
            match key {
                "inline-stdout" => {
                    call = call.inline_stdout(
                        value
                            .map(|v| arg_from_str(v, err, "inline-stdout", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "inline-stderr" => {
                    call = call.inline_stderr(
                        value
                            .map(|v| arg_from_str(v, err, "inline-stderr", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "inline-output-files" => {
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(
                                    ["inline-output-files", "inline-stderr", "inline-stdout"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
    ...
    async fn _action_results_update(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            {
                "execution-metadata.execution-completed-timestamp" => Some((
                    "executionMetadata.executionCompletedTimestamp",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "execution-metadata.execution-start-timestamp" => Some((
                    "executionMetadata.executionStartTimestamp",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "execution-metadata.input-fetch-completed-timestamp" => Some((
                    "executionMetadata.inputFetchCompletedTimestamp",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "execution-metadata.input-fetch-start-timestamp" => Some((
                    "executionMetadata.inputFetchStartTimestamp",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "execution-metadata.output-upload-completed-timestamp" => Some((
                    "executionMetadata.outputUploadCompletedTimestamp",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "execution-metadata.output-upload-start-timestamp" => Some((
                    "executionMetadata.outputUploadStartTimestamp",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "execution-metadata.queued-timestamp" => Some((
                    "executionMetadata.queuedTimestamp",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "execution-metadata.worker" => Some((
                    "executionMetadata.worker",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "execution-metadata.worker-completed-timestamp" => Some((
                    "executionMetadata.workerCompletedTimestamp",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "execution-metadata.worker-start-timestamp" => Some((
                    "executionMetadata.workerStartTimestamp",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                ...
                "stderr-digest.size-bytes" => Some((
                    "stderrDigest.sizeBytes",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                ...
                "stdout-digest.size-bytes" => Some((
                    "stdoutDigest.sizeBytes",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        ...
                        &vec![
                            ...
                            "exit-code",
                            "hash",
                            "input-fetch-completed-timestamp",
                            "input-fetch-start-timestamp",
                            "output-upload-completed-timestamp",
                            "output-upload-start-timestamp",
                            "queued-timestamp",
                            "size-bytes",
                            "stderr-digest",
                            "stderr-raw",
            ...
            .iter()
        {
            ...
            match key {
                "results-cache-policy-priority" => {
                    call = call.results_cache_policy_priority(
                        value
                            .map(|v| arg_from_str(v, err, "results-cache-policy-priority", "int32"))
                            .unwrap_or(-0),
                    );
                }
                _ => {
    ...
    async fn _actions_execute(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "action-digest.size-bytes" => Some((
                    "actionDigest.sizeBytes",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "execution-policy.priority" => Some((
                ...
                "results-cache-policy.priority" => Some((
        ...
        let mut request: api::BuildBazelRemoteExecutionV2ExecuteRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .actions()
            .execute(request, opt.value_of("instance-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _blobs_batch_read(
        ...
        let mut request: api::BuildBazelRemoteExecutionV2BatchReadBlobsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .blobs()
            .batch_read(request, opt.value_of("instance-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _blobs_batch_update(
        ...
        let mut request: api::BuildBazelRemoteExecutionV2BatchUpdateBlobsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .blobs()
            .batch_update(request, opt.value_of("instance-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _blobs_find_missing(
        ...
        let mut request: api::BuildBazelRemoteExecutionV2FindMissingBlobsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .blobs()
            .find_missing(request, opt.value_of("instance-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _methods_get_capabilities(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .methods()
            .get_capabilities(opt.value_of("instance-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _operations_wait_execution(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::BuildBazelRemoteExecutionV2WaitExecutionRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .operations()
            .wait_execution(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("action-results", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("action-results".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("actions", Some(opt)) => match opt.subcommand() {
                ("execute", Some(opt)) => {
                    call_result = self._actions_execute(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("actions".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("blobs", Some(opt)) => match opt.subcommand() {
                ("batch-read", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("blobs".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ...
            ("operations", Some(opt)) => match opt.subcommand() {
                ("wait-execution", Some(opt)) => {
                    call_result = self
                        ._operations_wait_execution(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "remotebuildexecution2-secret.json",
