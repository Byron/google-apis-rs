...
use google_doubleclickbidmanager1d1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _queries_createquery(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "params.options.include-only-targeted-user-lists" => Some((
                ...
                "params.options.path-query-options.channel-grouping.fallback-name" => Some((
                ...
                "params.options.path-query-options.channel-grouping.name" => Some((
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        ...
                        &vec![
                            ...
                            "group-bys",
                            "include-invite-data",
                            "include-only-targeted-user-lists",
                            "kind",
            ...
            .iter()
        {
            ...
            match key {
                "asynchronous" => {
                    call = call.asynchronous(
                        value
                            .map(|v| arg_from_str(v, err, "asynchronous", "boolean"))
                            .unwrap_or(false),
                    );
                }
                _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "doubleclickbidmanager1d1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/doubleclickbidmanager1d1", config_dir))
        .build()
        .await
        .unwrap();
