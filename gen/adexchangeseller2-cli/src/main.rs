...
use google_adexchangeseller2::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _accounts_metadata_dimensions_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .metadata_dimensions_list(opt.value_of("account-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_metadata_metrics_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .metadata_metrics_list(opt.value_of("account-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_preferreddeals_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.accounts().preferreddeals_get(
            opt.value_of("account-id").unwrap_or(""),
    ...
    async fn _accounts_preferreddeals_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .preferreddeals_list(opt.value_of("account-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
        ...
        match self.opt.subcommand() {
            ("accounts", Some(opt)) => match opt.subcommand() {
                ...
                ("metadata-dimensions-list", Some(opt)) => {
                    call_result = self
                        ._accounts_metadata_dimensions_list(opt, dry_run, &mut err)
                        .await;
                }
                ("metadata-metrics-list", Some(opt)) => {
                    call_result = self
                        ._accounts_metadata_metrics_list(opt, dry_run, &mut err)
                        .await;
                }
                ("preferreddeals-get", Some(opt)) => {
                    call_result = self
                        ._accounts_preferreddeals_get(opt, dry_run, &mut err)
                        .await;
                }
                ("preferreddeals-list", Some(opt)) => {
                    call_result = self
                        ._accounts_preferreddeals_list(opt, dry_run, &mut err)
                        .await;
                }
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "adexchangeseller2-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/adexchangeseller2", config_dir))
        .build()
        .await
        .unwrap();
