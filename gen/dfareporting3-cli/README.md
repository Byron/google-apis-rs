...
DO NOT EDIT !
-->
The `dfareporting3` command-line interface *(CLI)* allows to use most features of the *Google dfareporting* service from the comfort of your terminal.
...
Find the source code [on github](https://github.com/Byron/google-apis-rs/tree/master/gen/dfareporting3-cli).

# Usage

This documentation was generated from the *dfareporting* API at revision *20180830*. The CLI is at version *1.0.12*.

```bash
dfareporting3 [options]
...
The program will store all persistent data in the `~/.google-service-cli` directory in *JSON* files prefixed with `dfareporting3-`.  You can change the directory used to store configuration with the `--config-dir` flag on a per-invocation basis.
...
directory, e.g. `~/.google-service-cli/dfareporting3-token-<scope-hash>.json`. No manual management of these tokens
is necessary.
...
`~/.google-service-cli/dfareporting3-secret.json`, assuming that the required *dfareporting* API 
...
You may consider redirecting standard error into a file for ease of use, e.g. `dfareporting3 --debug <resource> <method> [options] 2>debug.txt`.
