        ...
        match self.opt.subcommand() {
            ("mail", Some(opt)) => {
                match opt.subcommand() {
                    ("insert", Some(opt)) => {
                        call_result = self._mail_insert(opt, dry_run, &mut err);
                    },
                    _ => {
                        err.issues.push(CLIError::MissingMethodError("mail".to_string()));
                        writeln!(io::stderr(), "{}\n", opt.usage()).ok();
                    }
                }
            },
        ...
        ("mail", "methods: 'insert'", vec![
            ("insert",  
                  ...
                  vec![
                    (Some(r##"user-key"##),
                     None,
                     Some(r##"The email or immutable id of the user"##),
                     Some(true),
                     Some(false)),
