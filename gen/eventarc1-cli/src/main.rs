...
use google_eventarc1::{api, Error};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::Eventarc<hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>
    >,
    gp: Vec<&'static str>,
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["cloud-run", "create-time", "destination", "etag", "labels", "name", "path", "pubsub", "region", "service", "service-account", "subscription", "topic", "transport", "uid", "update-time"]);
                        err.issues.push(CLIError::Field(FieldError::Unknown(temp_cursor.to_string(), suggestion, value.map(|v| v.to_string()))));
                        None
                    }
                };
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["validate-only", "trigger-id"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["validate-only", "allow-missing", "etag"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["validate-only", "allow-missing", "update-mask"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
...
async fn main() {
    ...
    let arg_data = [
        ("projects", "methods: 'locations-get', 'locations-list', 'locations-operations-cancel', 'locations-operations-delete', 'locations-operations-get', 'locations-operations-list', 'locations-triggers-create', 'locations-triggers-delete', 'locations-triggers-get', 'locations-triggers-get-iam-policy', 'locations-triggers-list', 'locations-triggers-patch', 'locations-triggers-set-iam-policy' and 'locations-triggers-test-iam-permissions'", vec![
            ("locations-get",
            ...
            ("locations-triggers-patch",
                  ...
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Required. The resource name of the trigger. Must be unique within the location on the project and must be in `projects/{project}/locations/{location}/triggers/{trigger}` format."##),
                     Some(true),
                     Some(false)),
