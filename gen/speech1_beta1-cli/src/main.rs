...
extern crate mime;
extern crate strsim;
extern crate google_speech1_beta1 as api;
...
impl<'n> Engine<'n> {
    ...
    fn _speech_asyncrecognize(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    fn _speech_syncrecognize(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
            ...
            ("speech", Some(opt)) => {
                match opt.subcommand() {
                    ("asyncrecognize", Some(opt)) => {
                        call_result = self._speech_asyncrecognize(opt, dry_run, &mut err);
                    },
                    ("syncrecognize", Some(opt)) => {
                        call_result = self._speech_syncrecognize(opt, dry_run, &mut err);
                    },
                    _ => {
    ...
    fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match cmn::application_secret_from_directory(&config_dir, "speech1-beta1-secret.json",
