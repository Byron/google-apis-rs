...
use google_speech1_beta1::{api, Error};
...
impl<'n> Engine<'n> {
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["name", "page-size", "filter", "page-token"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
    ...
    async fn _speech_asyncrecognize(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    "config.sample-rate" => Some(("config.sampleRate", JsonTypeInfo { jtype: JsonType::Int, ctype: ComplexType::Pod })),
                    ...
                    "config.speech-context.phrases" => Some(("config.speechContext.phrases", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Vec })),
                    ...
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["audio", "config", "content", "encoding", "language-code", "max-alternatives", "phrases", "profanity-filter", "sample-rate", "speech-context", "uri"]);
        ...
        let mut request: api::AsyncRecognizeRequest = json::value::from_value(object).unwrap();
        let mut call = self.hub.speech().asyncrecognize(request);
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
    ...
    async fn _speech_syncrecognize(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut request: api::SyncRecognizeRequest = json::value::from_value(object).unwrap();
        let mut call = self.hub.speech().syncrecognize(request);
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
    ...
    async fn _doit(&self, dry_run: bool) -> Result<Result<(), DoitError>, Option<InvalidOptionsError>> {
        ...
        match self.opt.subcommand() {
            ...
            ("speech", Some(opt)) => {
                match opt.subcommand() {
                    ("asyncrecognize", Some(opt)) => {
                        call_result = self._speech_asyncrecognize(opt, dry_run, &mut err).await;
                    },
                    ("syncrecognize", Some(opt)) => {
                        call_result = self._speech_syncrecognize(opt, dry_run, &mut err).await;
                    },
                    _ => {
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "speech1-beta1-secret.json",
        ...
        ).persist_tokens_to_disk(format!("{}/speech1-beta1", config_dir)).build().await.unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ...
        service."##),
                    "Details at http://byron.github.io/google-apis-rs/google_speech1_beta1_cli/operations_get",
                  vec![
                    (Some(r##"name"##),
                     None,
        ...
        is the parent resource, without the operations collection id."##),
                    "Details at http://byron.github.io/google-apis-rs/google_speech1_beta1_cli/operations_list",
                  vec![
                    (Some(r##"v"##),
                     Some(r##"p"##),
                     Some(r##"Set various optional parameters, matching the key=value form"##),
        ...
        ("speech", "methods: 'asyncrecognize' and 'syncrecognize'", vec![
            ("asyncrecognize",
        ...
        an `AsyncRecognizeResponse` message."##),
                    "Details at http://byron.github.io/google-apis-rs/google_speech1_beta1_cli/speech_asyncrecognize",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
        ...
        has been sent and processed."##),
                    "Details at http://byron.github.io/google-apis-rs/google_speech1_beta1_cli/speech_syncrecognize",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
                     Some(r##"Set various fields of the request structure, matching the key=value form"##),
