...
use google_resourceviews1_beta2::{api, Error};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::Resourceviews<hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>
    >,
    gp: Vec<&'static str>,
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["max-results", "page-token", "filter"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["max-results", "service-name", "format", "page-token", "list-state"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
