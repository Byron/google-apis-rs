...
use google_resourceviews1_beta2::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _zone_views_add_resources(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::ZoneViewsAddResourcesRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.zone_views().add_resources(
            request,
            opt.value_of("project").unwrap_or(""),
            opt.value_of("zone").unwrap_or(""),
            opt.value_of("resource-view").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
    ...
    async fn _zone_views_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.zone_views().delete(
            opt.value_of("project").unwrap_or(""),
            opt.value_of("zone").unwrap_or(""),
            opt.value_of("resource-view").unwrap_or(""),
        );
    ...
    async fn _zone_views_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.zone_views().get(
            opt.value_of("project").unwrap_or(""),
            opt.value_of("zone").unwrap_or(""),
            opt.value_of("resource-view").unwrap_or(""),
        );
    ...
    async fn _zone_views_get_service(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.zone_views().get_service(
            opt.value_of("project").unwrap_or(""),
            opt.value_of("zone").unwrap_or(""),
            opt.value_of("resource-view").unwrap_or(""),
        );
    ...
    async fn _zone_views_insert(
        ...
        let mut request: api::ResourceView = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.zone_views().insert(
            request,
            opt.value_of("project").unwrap_or(""),
            opt.value_of("zone").unwrap_or(""),
        );
    ...
    async fn _zone_views_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.zone_views().list(
            opt.value_of("project").unwrap_or(""),
            opt.value_of("zone").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _zone_views_list_resources(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.zone_views().list_resources(
            opt.value_of("project").unwrap_or(""),
            opt.value_of("zone").unwrap_or(""),
            opt.value_of("resource-view").unwrap_or(""),
        );
    ...
    async fn _zone_views_remove_resources(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::ZoneViewsRemoveResourcesRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.zone_views().remove_resources(
            request,
            opt.value_of("project").unwrap_or(""),
            opt.value_of("zone").unwrap_or(""),
            opt.value_of("resource-view").unwrap_or(""),
    ...
    async fn _zone_views_set_service(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion =
                        FieldCursor::did_you_mean(key, &vec!["fingerprint", "resource-name"]);
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
                        value.map(|v| v.to_string()),
        ...
        let mut request: api::ZoneViewsSetServiceRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.zone_views().set_service(
            request,
            opt.value_of("project").unwrap_or(""),
            opt.value_of("zone").unwrap_or(""),
            opt.value_of("resource-view").unwrap_or(""),
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ...
            ("zone-views", Some(opt)) => match opt.subcommand() {
                ...
                ("list-resources", Some(opt)) => {
                    call_result = self
                        ._zone_views_list_resources(opt, dry_run, &mut err)
                        .await;
                }
                ("remove-resources", Some(opt)) => {
                    call_result = self
                        ._zone_views_remove_resources(opt, dry_run, &mut err)
                        .await;
                }
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("zone-views".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "resourceviews1-beta2-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/resourceviews1-beta2", config_dir))
        .build()
        .await
        .unwrap();
