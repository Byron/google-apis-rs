...
impl<'n, 'a> Engine<'n, 'a> {
    ...
    fn _zone_views_add_resources(&self, opt: &ArgMatches<'n, 'a>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        let mut request = api::ZoneViewsAddResourcesRequest::default();
    ...
    fn _zone_views_insert(&self, opt: &ArgMatches<'n, 'a>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        let mut request = api::ResourceView::default();
        let mut field_cursor = FieldCursor::default();
        for kvarg in opt.values_of("kv").unwrap_or(Vec::new()).iter() {
            let last_errc = err.issues.len();
            let (key, value) = parse_kv_arg(&*kvarg, err, false);
        ...
        for parg in opt.values_of("v").unwrap_or(Vec::new()).iter() {
            ...
            match key {
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                Vec::new() + &self.gp + &["page-token", "service-name", "list-state", "max-results", "format"]
                                                            ));
                    }
                }
            }
    ...
    fn _zone_views_remove_resources(&self, opt: &ArgMatches<'n, 'a>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        let mut request = api::ZoneViewsRemoveResourcesRequest::default();
        let mut field_cursor = FieldCursor::default();
        for kvarg in opt.values_of("kv").unwrap_or(Vec::new()).iter() {
            let last_errc = err.issues.len();
            let (key, value) = parse_kv_arg(&*kvarg, err, false);
    ...
    fn _zone_views_set_service(&self, opt: &ArgMatches<'n, 'a>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        let mut request = api::ZoneViewsSetServiceRequest::default();
        let mut field_cursor = FieldCursor::default();
        for kvarg in opt.values_of("kv").unwrap_or(Vec::new()).iter() {
            let last_errc = err.issues.len();
            let (key, value) = parse_kv_arg(&*kvarg, err, false);
            ...
            ("zone-views", Some(opt)) => {
                match opt.subcommand() {
                    ("add-resources", Some(opt)) => {
                        call_result = self._zone_views_add_resources(opt, dry_run, &mut err);
                    },
                    ("delete", Some(opt)) => {
                        call_result = self._zone_views_delete(opt, dry_run, &mut err);
                    },
                    ("get", Some(opt)) => {
                        call_result = self._zone_views_get(opt, dry_run, &mut err);
                    },
                    ("get-service", Some(opt)) => {
                        call_result = self._zone_views_get_service(opt, dry_run, &mut err);
                    },
                    ("insert", Some(opt)) => {
                        call_result = self._zone_views_insert(opt, dry_run, &mut err);
                    },
                    ("list", Some(opt)) => {
                        call_result = self._zone_views_list(opt, dry_run, &mut err);
                    },
                    ("list-resources", Some(opt)) => {
                        call_result = self._zone_views_list_resources(opt, dry_run, &mut err);
                    },
                    ("remove-resources", Some(opt)) => {
                        call_result = self._zone_views_remove_resources(opt, dry_run, &mut err);
                    },
                    ("set-service", Some(opt)) => {
                        call_result = self._zone_views_set_service(opt, dry_run, &mut err);
                    },
                    _ => {
