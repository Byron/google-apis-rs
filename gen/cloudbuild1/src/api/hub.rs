...
impl<'a, S> client::Hub for CloudBuild<S> {}

impl<'a, S> CloudBuild<S> {

    pub fn new<A: 'static + client::GetToken>(client: hyper::Client<S, hyper::body::Body>, auth: A) -> CloudBuild<S> {
        CloudBuild {
            client,
            auth: Box::new(auth),
    ...
    pub fn github_dot_com_webhook(&'a self) -> GithubDotComWebhookMethods<'a, S> {
        GithubDotComWebhookMethods { hub: &self }
    }
