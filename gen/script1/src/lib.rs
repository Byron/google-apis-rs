...
//! The original source code is [on github](https://github.com/Byron/google-apis-rs/tree/main/gen/script1).
//! # Features
//!
//! Handle the following *Resources* with ease from the central [hub](Script) ...
//!
//! * processes
//!  * [*list*](api::ProcessListCall) and [*list script processes*](api::ProcessListScriptProcessCall)
//! * [projects](api::Project)
//!  * [*create*](api::ProjectCreateCall), [*deployments create*](api::ProjectDeploymentCreateCall), [*deployments delete*](api::ProjectDeploymentDeleteCall), [*deployments get*](api::ProjectDeploymentGetCall), [*deployments list*](api::ProjectDeploymentListCall), [*deployments update*](api::ProjectDeploymentUpdateCall), [*get*](api::ProjectGetCall), [*get content*](api::ProjectGetContentCall), [*get metrics*](api::ProjectGetMetricCall), [*update content*](api::ProjectUpdateContentCall), [*versions create*](api::ProjectVersionCreateCall), [*versions get*](api::ProjectVersionGetCall) and [*versions list*](api::ProjectVersionListCall)
//! * scripts
//!  * [*run*](api::ScriptRunCall)
//!
//!
//!
//!
...
//!
//! ```ignore
//! let r = hub.projects().deployments_create(...).doit().await
//! let r = hub.projects().deployments_delete(...).doit().await
//! let r = hub.projects().deployments_get(...).doit().await
//! let r = hub.projects().deployments_list(...).doit().await
//! let r = hub.projects().deployments_update(...).doit().await
//! let r = hub.projects().versions_create(...).doit().await
//! let r = hub.projects().versions_get(...).doit().await
//! let r = hub.projects().versions_list(...).doit().await
//! let r = hub.projects().create(...).doit().await
//! let r = hub.projects().get(...).doit().await
//! let r = hub.projects().get_content(...).doit().await
//! let r = hub.projects().get_metrics(...).doit().await
//! let r = hub.projects().update_content(...).doit().await
//! ```
//!
...
//! # async fn dox() {
//! use script1::{Script, FieldMask, hyper_rustls, hyper_util, yup_oauth2};
//!
...
//!         .build()
//! );
//! let mut hub = Script::new(client, auth);
//! // You can configure optional parameters by calling the respective setters at will, and
//! // execute the final call using `doit()`.
//! // Values shown here are possibly random and not representative !
//! let result = hub.projects().deployments_list("scriptId")
//!              .page_token("voluptua.")
//!              .page_size(-27)
//!              .doit().await;
//!
