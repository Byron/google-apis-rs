...
use google_vectortile1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
            ...
            .iter()
        {
            ...
            match key {
                ...
                "enable-unclipped-buildings" => {
                    call = call.enable_unclipped_buildings(
                        value
                            .map(|v| arg_from_str(v, err, "enable-unclipped-buildings", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "enable-private-roads" => {
                    call = call.enable_private_roads(
                        value
                            .map(|v| arg_from_str(v, err, "enable-private-roads", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "enable-political-features" => {
                    call = call.enable_political_features(
                        value
                            .map(|v| arg_from_str(v, err, "enable-political-features", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "enable-modeled-volumes" => {
                    call = call.enable_modeled_volumes(
                        value
                            .map(|v| arg_from_str(v, err, "enable-modeled-volumes", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "enable-feature-names" => {
                    call = call.enable_feature_names(
                        value
                            .map(|v| arg_from_str(v, err, "enable-feature-names", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "enable-detailed-highway-types" => {
                    call = call.enable_detailed_highway_types(
                        value
                            .map(|v| {
                                arg_from_str(v, err, "enable-detailed-highway-types", "boolean")
                            })
                            .unwrap_or(false),
                    );
                }
                ...
                "always-include-building-footprints" => {
                    call = call.always_include_building_footprints(
                        value
                            .map(|v| {
                                arg_from_str(
                                    v,
            ...
            .iter()
        {
            ...
            match key {
                ...
                "min-elevation-resolution-cells" => {
                    call = call.min_elevation_resolution_cells(
                        value
                            .map(|v| {
                                arg_from_str(v, err, "min-elevation-resolution-cells", "int32")
                            })
                            .unwrap_or(-0),
                    );
                }
                "max-elevation-resolution-cells" => {
                    call = call.max_elevation_resolution_cells(
                        value
                            .map(|v| {
                                arg_from_str(v, err, "max-elevation-resolution-cells", "int32")
                            })
                            .unwrap_or(-0),
                    );
                }
                ...
                "altitude-precision-centimeters" => {
                    call = call.altitude_precision_centimeters(
                        value
                            .map(|v| {
                                arg_from_str(v, err, "altitude-precision-centimeters", "int32")
                            })
                            .unwrap_or(-0),
                    );
                }
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("featuretiles", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._featuretiles_get(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("featuretiles".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("terraintiles", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._terraintiles_get(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("terraintiles".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "vectortile1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/vectortile1", config_dir))
        .build()
        .await
        .unwrap();
