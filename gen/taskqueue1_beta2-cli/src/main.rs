...
use google_taskqueue1_beta2::{api, yup_oauth2, Error};
...
{
    async fn _taskqueues_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.taskqueues().get(
            opt.value_of("project").unwrap_or(""),
            opt.value_of("taskqueue").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            ...
            .iter()
        {
            ...
            match key {
                "get-stats" => {
                    call = call.get_stats(
                        value
                            .map(|v| arg_from_str(v, err, "get-stats", "boolean"))
                            .unwrap_or(false),
                    );
                }
                _ => {
    ...
    async fn _tasks_insert(
        ...
        let mut call = self.hub.tasks().insert(
            request,
            opt.value_of("project").unwrap_or(""),
            opt.value_of("taskqueue").unwrap_or(""),
        );
    ...
    async fn _tasks_lease(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let num_tasks: i32 = arg_from_str(
            ...
            .iter()
        {
            ...
            match key {
                ...
                "group-by-tag" => {
                    call = call.group_by_tag(
                        value
                            .map(|v| arg_from_str(v, err, "group-by-tag", "boolean"))
                            .unwrap_or(false),
                    );
                }
                _ => {
    ...
    async fn _tasks_list(
        ...
        let mut call = self.hub.tasks().list(
            opt.value_of("project").unwrap_or(""),
            opt.value_of("taskqueue").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _tasks_patch(
        ...
        let mut request: api::Task = serde_json::value::from_value(object).unwrap();
        let new_lease_seconds: i32 = arg_from_str(
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("taskqueues", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._taskqueues_get(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("taskqueues".to_string()));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "taskqueue1-beta2-secret.json",
