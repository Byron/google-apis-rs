...
use google_sql1_beta4::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
            ...
            .iter()
        {
            ...
            match key {
                "verify-connection-only" => {
                    call = call.verify_connection_only(
                        value
                            .map(|v| arg_from_str(v, err, "verify-connection-only", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "sync-mode" => {
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["sync-mode", "verify-connection-only"].iter().map(|v| *v),
                                );
                                v
                            }));
                    }
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "sql1-beta4-secret.json",
