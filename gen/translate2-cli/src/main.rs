...
use google_translate2::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _detections_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.detections().list(
            &opt.values_of("q")
                .map(|i| i.collect())
                .unwrap_or(Vec::new())
                .iter()
    ...
    async fn _translations_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.translations().list(
            &opt.values_of("q")
                .map(|i| i.collect())
                .unwrap_or(Vec::new())
                .iter()
    ...
    async fn _translations_translate(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["format", "model", "q", "source", "target"],
                    );
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("detections", Some(opt)) => match opt.subcommand() {
                ("detect", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("detections".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("languages", Some(opt)) => match opt.subcommand() {
                ("list", Some(opt)) => {
                    call_result = self._languages_list(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("languages".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("translations", Some(opt)) => match opt.subcommand() {
                ("list", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("translations".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "translate2-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/translate2", config_dir))
        .build()
        .await
        .unwrap();
