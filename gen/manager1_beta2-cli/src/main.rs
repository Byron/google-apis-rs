...
use google_manager1_beta2::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _deployments_delete(
        ...
        let mut call = self.hub.deployments().delete(
            ...
            opt.value_of("deployment-name").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
    ...
    async fn _deployments_insert(
        ...
        let mut call = self.hub.deployments().insert(
            request,
            opt.value_of("project-id").unwrap_or(""),
            opt.value_of("region").unwrap_or(""),
        );
    ...
    async fn _deployments_list(
        ...
        let mut call = self.hub.deployments().list(
            opt.value_of("project-id").unwrap_or(""),
            opt.value_of("region").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _templates_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.templates().delete(
            opt.value_of("project-id").unwrap_or(""),
    ...
    async fn _templates_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.templates().get(
            opt.value_of("project-id").unwrap_or(""),
            opt.value_of("template-name").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ...
            ("templates", Some(opt)) => match opt.subcommand() {
                ("delete", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("templates".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "manager1-beta2-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/manager1-beta2", config_dir))
        .build()
        .await
        .unwrap();
