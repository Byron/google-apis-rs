...
impl<'n, 'a> Engine<'n, 'a> {
    ...
    fn _deployments_insert(&self, opt: &ArgMatches<'n, 'a>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        let mut request = api::Deployment::default();
        ...
        for kvarg in opt.values_of("kv").unwrap_or(Vec::new()).iter() {
            ...
            match &temp_cursor.to_string()[..] {
                "state.status" => {
                        ...
                        request.state.as_mut().unwrap().status = Some(value.unwrap_or("").to_string());
                    },
                "state.details" => {
                        request_state_init(&mut request);
                        request.state.as_mut().unwrap().details = Some(value.unwrap_or("").to_string());
                    },
                "creation-date" => {
                        request_state_init(&mut request);
    ...
    fn _doit(&self, dry_run: bool) -> Result<Result<(), DoitError>, Option<InvalidOptionsError>> {
        ...
        match self.opt.subcommand() {
            ...
            ("templates", Some(opt)) => {
                match opt.subcommand() {
                    ("delete", Some(opt)) => {
                        call_result = self._templates_delete(opt, dry_run, &mut err);
                    },
                    ("get", Some(opt)) => {
                        call_result = self._templates_get(opt, dry_run, &mut err);
                    },
                    ("insert", Some(opt)) => {
                        call_result = self._templates_insert(opt, dry_run, &mut err);
                    },
                    ("list", Some(opt)) => {
                        call_result = self._templates_list(opt, dry_run, &mut err);
                    },
                    _ => {
