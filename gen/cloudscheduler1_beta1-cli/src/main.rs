...
use google_cloudscheduler1_beta1::{api, Error, oauth2, client::chrono, FieldMask};
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                "legacy-app-engine-cron" => {
                    call = call.legacy_app_engine_cron(        value.map(|v| arg_from_str(v, err, "legacy-app-engine-cron", "boolean")).unwrap_or(false));
                },
                _ => {
                    let mut found = false;
                    for param in &self.gp {
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["filter", "legacy-app-engine-cron", "page-size", "page-token"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
