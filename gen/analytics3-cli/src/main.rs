...
use google_analytics3::{api, Error};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::Analytics<hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>
    >,
    gp: Vec<&'static str>,
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           ...
                                                                           v.extend(["output", "segment", "include-empty-rows", "sort", "sampling-level", "max-results", "filters", "dimensions", "start-index"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["sort", "max-results", "sampling-level", "filters", "dimensions", "start-index"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
