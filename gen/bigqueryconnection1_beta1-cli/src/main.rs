...
use google_bigqueryconnection1_beta1::{api, Error};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::BigQueryConnectionService<hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>
    >,
    gp: Vec<&'static str>,
...
impl<'n> Engine<'n> {
    ...
    async fn _projects_locations_connections_update_credential(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
            ...
            ("projects", Some(opt)) => {
                match opt.subcommand() {
                    ...
                    ("locations-connections-update-credential", Some(opt)) => {
                        call_result = self._projects_locations_connections_update_credential(opt, dry_run, &mut err).await;
                    },
                    _ => {
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "bigqueryconnection1-beta1-secret.json",
        ...
        ).persist_tokens_to_disk(format!("{}/bigqueryconnection1-beta1", config_dir)).build().await.unwrap();
