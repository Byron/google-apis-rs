...
use google_datafusion1::{api, Error};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::DataFusion<hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>
    >,
    gp: Vec<&'static str>,
...
impl<'n> Engine<'n> {
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["api-endpoint", "create-time", "dataproc-service-account", "description", "display-name", "enable-rbac", "enable-stackdriver-logging", "enable-stackdriver-monitoring", "gcs-bucket", "ip-allocation", "labels", "name", "network", "network-config", "options", "p4-service-account", "private-instance", "service-account", "service-endpoint", "state", "state-message", "tenant-project-id", "type", "update-time", "version", "zone"]);
    ...
    async fn _projects_locations_instances_get_iam_policy(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    async fn _projects_locations_instances_restart(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut request: api::RestartInstanceRequest = json::value::from_value(object).unwrap();
        let mut call = self.hub.projects().locations_instances_restart(request, opt.value_of("name").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
    ...
    async fn _projects_locations_instances_set_iam_policy(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    async fn _projects_locations_instances_test_iam_permissions(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["page-token", "include-unrevealed-locations", "filter", "page-size"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
    ...
    async fn _projects_locations_versions_list(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["latest-patch-only", "page-token", "page-size"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
            ...
            ("projects", Some(opt)) => {
                match opt.subcommand() {
                    ...
                    ("locations-instances-get-iam-policy", Some(opt)) => {
                        call_result = self._projects_locations_instances_get_iam_policy(opt, dry_run, &mut err).await;
                    },
                    ...
                    ("locations-instances-restart", Some(opt)) => {
                        call_result = self._projects_locations_instances_restart(opt, dry_run, &mut err).await;
                    },
                    ("locations-instances-set-iam-policy", Some(opt)) => {
                        call_result = self._projects_locations_instances_set_iam_policy(opt, dry_run, &mut err).await;
                    },
                    ("locations-instances-test-iam-permissions", Some(opt)) => {
                        call_result = self._projects_locations_instances_test_iam_permissions(opt, dry_run, &mut err).await;
                    },
                    ...
                    ("locations-versions-list", Some(opt)) => {
                        call_result = self._projects_locations_versions_list(opt, dry_run, &mut err).await;
                    },
                    _ => {
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "datafusion1-secret.json",
        ...
        ).persist_tokens_to_disk(format!("{}/datafusion1", config_dir)).build().await.unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ("projects", "methods: 'locations-get', 'locations-instances-create', 'locations-instances-delete', 'locations-instances-get', 'locations-instances-get-iam-policy', 'locations-instances-list', 'locations-instances-patch', 'locations-instances-restart', 'locations-instances-set-iam-policy', 'locations-instances-test-iam-permissions', 'locations-list', 'locations-operations-cancel', 'locations-operations-delete', 'locations-operations-get', 'locations-operations-list' and 'locations-versions-list'", vec![
            ("locations-get",
            ...
            ("locations-instances-create",
                  ...
                  vec![
                    (Some(r##"parent"##),
                     None,
                     Some(r##"The instance's project and location in the format projects/{project}/locations/{location}."##),
                     Some(true),
                     Some(false)),
            ...
            ("locations-instances-delete",
                  ...
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"The instance resource name in the format projects/{project}/locations/{location}/instances/{instance}"##),
                     Some(true),
                     Some(false)),
            ...
            ("locations-instances-list",
                  ...
                  vec![
                    (Some(r##"parent"##),
                     None,
                     Some(r##"The project and location for which to retrieve instance information in the format projects/{project}/locations/{location}. If the location is specified as '-' (wildcard), then all regions available to the project are queried, and the results are aggregated."##),
                     Some(true),
                     Some(false)),
            ...
            ("locations-instances-restart",
                  ...
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Name of the Data Fusion instance which need to be restarted in the form of projects/{project}/locations/{location}/instances/{instance}"##),
                     Some(true),
                     Some(false)),
