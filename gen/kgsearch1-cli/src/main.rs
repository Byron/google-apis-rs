...
extern crate mime;
extern crate strsim;
extern crate google_kgsearch1 as api;
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::Kgsearch<hyper::Client, Authenticator<DefaultAuthenticatorDelegate, JsonTokenStorage, hyper::Client>>,
    gp: Vec<&'static str>,
...
impl<'n> Engine<'n> {
    fn _entities_search(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.entities().search();
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                ...
                "prefix" => {
                    call = call.prefix(arg_from_str(value.unwrap_or("false"), err, "prefix", "boolean"));
                },
                "limit" => {
                    call = call.limit(arg_from_str(value.unwrap_or("-0"), err, "limit", "integer"));
                },
                "languages" => {
                    call = call.add_languages(value.unwrap_or(""));
                },
                "indent" => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           ...
                                                                           v.extend(["indent", "ids", "languages", "prefix", "limit", "query", "types"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
        ...
        match self.opt.subcommand() {
            ("entities", Some(opt)) => {
                match opt.subcommand() {
                    ("search", Some(opt)) => {
                        call_result = self._entities_search(opt, dry_run, &mut err);
                    },
                    _ => {
                        err.issues.push(CLIError::MissingMethodError("entities".to_string()));
                        writeln!(io::stderr(), "{}\n", opt.usage()).ok();
                    }
                }
            },
    ...
    fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match cmn::application_secret_from_directory(&config_dir, "kgsearch1-secret.json",
        ...
        let engine = Engine {
            opt: opt,
            hub: api::Kgsearch::new(client, auth),
