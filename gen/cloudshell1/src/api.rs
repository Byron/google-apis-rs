...
/// The list links the activity name, along with information about where it is used (one of *request* and *response*).
///
/// * [environments generate access token users](UserEnvironmentGenerateAccessTokenCall) (response)
#[cfg_attr(feature = "utoipa", derive(utoipa::ToSchema))]
#[serde_with::serde_as]
#[derive(Default, Clone, Debug, serde::Serialize, serde::Deserialize)]
...
/// let mut hub = CloudShell::new(client, auth);
/// // Usually you wouldn't bind this to a variable, but keep calling *CallBuilders*
/// // like `environments_add_public_key(...)`, `environments_authorize(...)`, `environments_generate_access_token(...)`, `environments_get(...)`, `environments_remove_public_key(...)` and `environments_start(...)`
/// // to build up your call.
/// let rb = hub.users();
/// # }
/// ```
...
impl<'a, C> UserMethods<'a, C> {
    ...
    pub fn environments_generate_access_token(
        &self,
        environment: &str,
    ) -> UserEnvironmentGenerateAccessTokenCall<'a, C> {
        UserEnvironmentGenerateAccessTokenCall {
            hub: self.hub,
            _environment: environment.to_string(),
            _ttl: Default::default(),
            _expire_time: Default::default(),
            _delegate: Default::default(),
            _additional_params: Default::default(),
            _scopes: Default::default(),
        }
...
/// Generates an access token for the user's environment.
///
/// A builder for the *environments.generateAccessToken* method supported by a *user* resource.
/// It is not used directly, but through a [`UserMethods`] instance.
///
/// # Example
///
...
/// // execute the final call using `doit()`.
/// // Values shown here are possibly random and not representative !
/// let result = hub.users().environments_generate_access_token("environment")
///              .ttl(chrono::Duration::seconds(6767418))
///              .expire_time(chrono::Utc::now())
///              .doit().await;
/// # }
/// ```
pub struct UserEnvironmentGenerateAccessTokenCall<'a, C>
where
    C: 'a,
{
    hub: &'a CloudShell<C>,
    ...
    _expire_time: Option<chrono::DateTime<chrono::offset::Utc>>,
    _delegate: Option<&'a mut dyn common::Delegate>,
    _additional_params: HashMap<String, String>,
    _scopes: BTreeSet<String>,
}

impl<'a, C> common::CallBuilder for UserEnvironmentGenerateAccessTokenCall<'a, C> {}

impl<'a, C> UserEnvironmentGenerateAccessTokenCall<'a, C>
where
    C: common::Connector,
{
    /// Perform the operation you have build so far.
    pub async fn doit(mut self) -> common::Result<(common::Response, GenerateAccessTokenResponse)> {
        ...
        for &field in ["alt", "environment", "ttl", "expireTime"].iter() {
            if self._additional_params.contains_key(field) {
                dlg.finished(false);
                return Err(common::Error::FieldClash(field));
            }
        ...
        params.push("environment", self._environment);
        if let Some(value) = self._ttl.as_ref() {
            params.push("ttl", common::serde::duration::to_string(&value));
        }
        if let Some(value) = self._expire_time.as_ref() {
            params.push("expireTime", common::serde::datetime_to_string(&value));
        }

        params.extend(self._additional_params.iter());

        params.push("alt", "json");
        let mut url = self.hub._base_url.clone() + "v1/{+environment}:generateAccessToken";
        if self._scopes.is_empty() {
            self._scopes
                .insert(Scope::CloudPlatform.as_ref().to_string());
        }
    ...
    /// Even though the property as already been set when instantiating this call,
    /// we provide this method for API completeness.
    pub fn environment(mut self, new_value: &str) -> UserEnvironmentGenerateAccessTokenCall<'a, C> {
        self._environment = new_value.to_string();
        self
    }
    ...
    ///
    /// Sets the *ttl* query property to the given value.
    pub fn ttl(
        mut self,
        new_value: chrono::Duration,
    ) -> UserEnvironmentGenerateAccessTokenCall<'a, C> {
        self._ttl = Some(new_value);
        self
    }
    ...
    ///
    /// Sets the *expire time* query property to the given value.
    pub fn expire_time(
        mut self,
        new_value: chrono::DateTime<chrono::offset::Utc>,
    ) -> UserEnvironmentGenerateAccessTokenCall<'a, C> {
        self._expire_time = Some(new_value);
        self
    }
    /// The delegate implementation is consulted whenever there is an intermediate result, or if something goes wrong
    /// while executing the actual API request.
    ...
    /// * *uploadType* (query-string) - Legacy upload protocol for media (e.g. "media", "multipart").
    /// * *upload_protocol* (query-string) - Upload protocol for media (e.g. "raw", "multipart").
    pub fn param<T>(mut self, name: T, value: T) -> UserEnvironmentGenerateAccessTokenCall<'a, C>
    where
        T: AsRef<str>,
    {
        self._additional_params
    ...
    /// encompass more rights than others. For example, for listing resources, a *read-only* scope will be
    /// sufficient, a read-write scope will do as well.
    pub fn add_scope<St>(mut self, scope: St) -> UserEnvironmentGenerateAccessTokenCall<'a, C>
    where
        St: AsRef<str>,
    {
        self._scopes.insert(String::from(scope.as_ref()));
    ...
    ///
    /// See [`Self::add_scope()`] for details.
    pub fn add_scopes<I, St>(mut self, scopes: I) -> UserEnvironmentGenerateAccessTokenCall<'a, C>
    where
        I: IntoIterator<Item = St>,
        St: AsRef<str>,
    {
    ...
    /// In this case, you have to specify your API-key using the `key` parameter (see [`Self::param()`]
    /// for details).
    pub fn clear_scopes(mut self) -> UserEnvironmentGenerateAccessTokenCall<'a, C> {
        self._scopes.clear();
        self
    }
}
