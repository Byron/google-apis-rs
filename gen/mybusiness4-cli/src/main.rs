...
use google_mybusiness4::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["admin-name", "name", "pending-invitation", "role"],
                    );
    ...
    async fn _accounts_create(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "organization-info.postal-address.address-lines" => Some((
                    "organizationInfo.postalAddress.addressLines",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Vec,
                    },
                )),
                "organization-info.postal-address.administrative-area" => Some((
                    "organizationInfo.postalAddress.administrativeArea",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "organization-info.postal-address.language-code" => Some((
                    "organizationInfo.postalAddress.languageCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "organization-info.postal-address.locality" => Some((
                    "organizationInfo.postalAddress.locality",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "organization-info.postal-address.organization" => Some((
                    "organizationInfo.postalAddress.organization",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "organization-info.postal-address.postal-code" => Some((
                    "organizationInfo.postalAddress.postalCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "organization-info.postal-address.recipients" => Some((
                    "organizationInfo.postalAddress.recipients",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Vec,
                    },
                )),
                "organization-info.postal-address.region-code" => Some((
                    "organizationInfo.postalAddress.regionCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "organization-info.postal-address.revision" => Some((
                    "organizationInfo.postalAddress.revision",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "organization-info.postal-address.sorting-code" => Some((
                    "organizationInfo.postalAddress.sortingCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "organization-info.postal-address.sublocality" => Some((
                    "organizationInfo.postalAddress.sublocality",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
    ...
    async fn _accounts_delete_notifications(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .delete_notifications(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_generate_account_number(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::GenerateAccountNumberRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .generate_account_number(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_get_notifications(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .get_notifications(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_list_recommend_google_locations(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .list_recommend_google_locations(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_admins_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_admins_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_admins_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_admins_delete(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_admins_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_admins_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_admins_patch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_admins_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_associate(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::AssociateLocationRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .locations_associate(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_batch_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::BatchGetLocationsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .locations_batch_get(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_batch_get_reviews(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                "ignore-rating-only-reviews" => Some((
        ...
        let mut request: api::BatchGetReviewsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .locations_batch_get_reviews(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_clear_association(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::ClearLocationAssociationRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .locations_clear_association(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_create(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "address.administrative-area" => Some((
                ...
                "location-key.explicit-no-place-id" => Some((
                    "locationKey.explicitNoPlaceId",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location-key.place-id" => Some((
                    "locationKey.placeId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                ...
                "location-key.plus-page-id" => Some((
                    "locationKey.plusPageId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location-key.request-id" => Some((
                ...
                "location-state.can-delete" => Some((
                ...
                "location-state.can-update" => Some((
                ...
                "location-state.has-pending-edits" => Some((
                ...
                "location-state.has-pending-verification" => Some((
                ...
                "location-state.is-disabled" => Some((
                    "locationState.isDisabled",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location-state.is-disconnected" => Some((
                ...
                "location-state.is-duplicate" => Some((
                ...
                "location-state.is-google-updated" => Some((
                ...
                "location-state.is-local-post-api-disabled" => Some((
                    "locationState.isLocalPostApiDisabled",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location-state.is-pending-review" => Some((
                ...
                "location-state.is-published" => Some((
                ...
                "location-state.is-suspended" => Some((
                ...
                "location-state.is-verified" => Some((
                ...
                "location-state.needs-reverification" => Some((
                ...
                "metadata.duplicate.access" => Some((
                ...
                "metadata.duplicate.location-name" => Some((
                    "metadata.duplicate.locationName",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "metadata.duplicate.place-id" => Some((
                    "metadata.duplicate.placeId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "metadata.maps-url" => Some((
                    "metadata.mapsUrl",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                ...
                "metadata.new-review-url" => Some((
                    "metadata.newReviewUrl",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                ...
                "primary-category.category-id" => Some((
                ...
                "primary-category.display-name" => Some((
                ...
                "service-area.radius.latlng.latitude" => Some((
                    "serviceArea.radius.latlng.latitude",
                    JsonTypeInfo {
                        jtype: JsonType::Float,
                        ctype: ComplexType::Pod,
                    },
                )),
                "service-area.radius.latlng.longitude" => Some((
                    "serviceArea.radius.latlng.longitude",
                    JsonTypeInfo {
                        jtype: JsonType::Float,
                        ctype: ComplexType::Pod,
                    },
                )),
                "service-area.radius.radius-km" => Some((
                    "serviceArea.radius.radiusKm",
                    JsonTypeInfo {
                        jtype: JsonType::Float,
                        ctype: ComplexType::Pod,
                    },
    ...
    async fn _accounts_locations_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_delete(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_fetch_verification_options(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_fetch_verification_options(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_find_matches(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["language-code", "max-cache-duration", "num-results"],
                    );
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
        ...
        let mut request: api::FindMatchingLocationsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .locations_find_matches(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_followers_get_metadata(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_followers_get_metadata(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _accounts_locations_get_google_updated(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_get_google_updated(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_local_posts_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "call-to-action.action-type" => Some((
                ...
                "event.schedule.end-date.day" => Some((
                    "event.schedule.endDate.day",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "event.schedule.end-date.month" => Some((
                    "event.schedule.endDate.month",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "event.schedule.end-date.year" => Some((
                    "event.schedule.endDate.year",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "event.schedule.end-time.hours" => Some((
                    "event.schedule.endTime.hours",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "event.schedule.end-time.minutes" => Some((
                    "event.schedule.endTime.minutes",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "event.schedule.end-time.nanos" => Some((
                    "event.schedule.endTime.nanos",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "event.schedule.end-time.seconds" => Some((
                    "event.schedule.endTime.seconds",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "event.schedule.start-date.day" => Some((
                    "event.schedule.startDate.day",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "event.schedule.start-date.month" => Some((
                    "event.schedule.startDate.month",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "event.schedule.start-date.year" => Some((
                    "event.schedule.startDate.year",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "event.schedule.start-time.hours" => Some((
                    "event.schedule.startTime.hours",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "event.schedule.start-time.minutes" => Some((
                    "event.schedule.startTime.minutes",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "event.schedule.start-time.nanos" => Some((
                    "event.schedule.startTime.nanos",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "event.schedule.start-time.seconds" => Some((
                    "event.schedule.startTime.seconds",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
        ...
        let mut request: api::LocalPost = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .locations_local_posts_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_local_posts_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_local_posts_delete(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_local_posts_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_local_posts_get(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_local_posts_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_local_posts_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_local_posts_patch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_local_posts_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_local_posts_report_insights(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                "basic-request.time-range.end-time" => Some((
                    "basicRequest.timeRange.endTime",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "basic-request.time-range.start-time" => Some((
                    "basicRequest.timeRange.startTime",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "local-post-names" => Some((
                    "localPostNames",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Vec,
        ...
        let mut request: api::ReportLocalPostInsightsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .locations_local_posts_report_insights(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_media_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "attribution.profile-photo-url" => Some((
                    "attribution.profilePhotoUrl",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "attribution.profile-url" => Some((
                ...
                "attribution.takedown-url" => Some((
                    "attribution.takedownUrl",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                ...
                "location-association.category" => Some((
                ...
                "location-association.price-list-item-id" => Some((
                    "locationAssociation.priceListItemId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_media_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_media_customers_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_media_customers_get(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_media_customers_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_media_customers_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_media_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_media_delete(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_media_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_media_get(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_media_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_media_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_media_patch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_media_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_media_start_upload(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::StartUploadMediaItemDataRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .locations_media_start_upload(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_patch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["attribute-mask", "update-mask", "validate-only"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
    ...
    async fn _accounts_locations_questions_answers_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_questions_answers_delete(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_questions_answers_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_questions_answers_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_questions_answers_upsert(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                "answer.author.display-name" => Some((
                ...
                "answer.author.profile-photo-url" => Some((
        ...
        let mut request: api::UpsertAnswerRequest = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .locations_questions_answers_upsert(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_questions_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::Question = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .locations_questions_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_questions_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_questions_delete(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_questions_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_questions_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
            .iter()
        {
            ...
            match key {
                ...
                "answers-per-question" => {
                    call = call.answers_per_question(
                        value
                            .map(|v| arg_from_str(v, err, "answers-per-question", "int32"))
                            .unwrap_or(-0),
                    );
                }
                _ => {
    ...
    async fn _accounts_locations_questions_patch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_questions_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_report_insights(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "driving-directions-request.language-code" => Some((
                ...
                "driving-directions-request.num-days" => Some((
                    "drivingDirectionsRequest.numDays",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
        ...
        let mut request: api::ReportLocationInsightsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .locations_report_insights(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_reviews_delete_reply(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_reviews_delete_reply(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_reviews_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_reviews_get(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_reviews_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_reviews_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_reviews_update_reply(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion =
                        FieldCursor::did_you_mean(key, &vec!["comment", "update-time"]);
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
                        value.map(|v| v.to_string()),
        ...
        let mut request: api::ReviewReply = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .locations_reviews_update_reply(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_transfer(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_transfer(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_verifications_complete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_verifications_complete(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_verifications_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_verifications_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_locations_verify(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                "address-input.mailer-contact-name" => Some((
                ...
                "email-input.email-address" => Some((
                ...
                "vetted-partner-input.token.token-string" => Some((
                    "vettedPartnerInput.token.tokenString",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
        ...
        let mut call = self
            .hub
            .accounts()
            .locations_verify(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_update_notifications(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["name", "notification-types", "topic-name"],
                    );
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
        ...
        let mut request: api::Notifications = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .update_notifications(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _chains_search(
            ...
            .iter()
        {
            ...
            match key {
                "result-count" => {
                    call = call.result_count(
                        value
                            .map(|v| arg_from_str(v, err, "result-count", "int32"))
                            .unwrap_or(-0),
                    );
                }
                "chain-display-name" => {
    ...
    async fn _google_locations_report(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "report-reason-bad-location" => Some((
                ...
                "report-reason-bad-recommendation" => Some((
                ...
                "report-reason-elaboration" => Some((
                ...
                "report-reason-language-code" => Some((
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        ...
                        &vec![
                            "location-group-name",
                            "report-reason-bad-location",
                            "report-reason-bad-recommendation",
                            "report-reason-elaboration",
                            "report-reason-language-code",
                        ],
                    );
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
        ...
        let mut request: api::ReportGoogleLocationRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .google_locations()
            .report(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _google_locations_search(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "location.additional-phones" => Some((
                ...
                "location.address.address-lines" => Some((
                ...
                "location.address.administrative-area" => Some((
                ...
                "location.address.language-code" => Some((
                    "location.address.languageCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.address.locality" => Some((
                ...
                "location.address.organization" => Some((
                ...
                "location.address.postal-code" => Some((
                ...
                "location.address.recipients" => Some((
                    "location.address.recipients",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Vec,
                    },
                )),
                "location.address.region-code" => Some((
                ...
                "location.address.revision" => Some((
                    "location.address.revision",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.address.sorting-code" => Some((
                    "location.address.sortingCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.address.sublocality" => Some((
                ...
                "location.location-key.explicit-no-place-id" => Some((
                    "location.locationKey.explicitNoPlaceId",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-key.place-id" => Some((
                ...
                "location.location-key.plus-page-id" => Some((
                    "location.locationKey.plusPageId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-key.request-id" => Some((
                    "location.locationKey.requestId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-name" => Some((
                ...
                "location.location-state.can-delete" => Some((
                    "location.locationState.canDelete",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-state.can-update" => Some((
                    "location.locationState.canUpdate",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-state.has-pending-edits" => Some((
                    "location.locationState.hasPendingEdits",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-state.has-pending-verification" => Some((
                    "location.locationState.hasPendingVerification",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-state.is-disabled" => Some((
                    "location.locationState.isDisabled",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-state.is-disconnected" => Some((
                    "location.locationState.isDisconnected",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-state.is-duplicate" => Some((
                    "location.locationState.isDuplicate",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-state.is-google-updated" => Some((
                    "location.locationState.isGoogleUpdated",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-state.is-local-post-api-disabled" => Some((
                    "location.locationState.isLocalPostApiDisabled",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-state.is-pending-review" => Some((
                    "location.locationState.isPendingReview",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-state.is-published" => Some((
                    "location.locationState.isPublished",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-state.is-suspended" => Some((
                    "location.locationState.isSuspended",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.location-state.is-verified" => Some((
                ...
                "location.location-state.needs-reverification" => Some((
                    "location.locationState.needsReverification",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.metadata.duplicate.access" => Some((
                ...
                "location.metadata.duplicate.location-name" => Some((
                    "location.metadata.duplicate.locationName",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.metadata.duplicate.place-id" => Some((
                    "location.metadata.duplicate.placeId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.metadata.maps-url" => Some((
                ...
                "location.metadata.new-review-url" => Some((
                ...
                "location.primary-category.category-id" => Some((
                ...
                "location.primary-category.display-name" => Some((
                ...
                "location.service-area.radius.latlng.latitude" => Some((
                    "location.serviceArea.radius.latlng.latitude",
                    JsonTypeInfo {
                        jtype: JsonType::Float,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.service-area.radius.latlng.longitude" => Some((
                    "location.serviceArea.radius.latlng.longitude",
                    JsonTypeInfo {
                        jtype: JsonType::Float,
                        ctype: ComplexType::Pod,
                    },
                )),
                "location.service-area.radius.radius-km" => Some((
                    "location.serviceArea.radius.radiusKm",
                    JsonTypeInfo {
                        jtype: JsonType::Float,
                        ctype: ComplexType::Pod,
                    },
    ...
    async fn _verification_tokens_generate(
        ...
        let mut request: api::GenerateVerificationTokenRequest =
            serde_json::value::from_value(object).unwrap();
        ...
        match self.opt.subcommand() {
            ("accounts", Some(opt)) => match opt.subcommand() {
                ...
                ("delete-notifications", Some(opt)) => {
                    call_result = self
                        ._accounts_delete_notifications(opt, dry_run, &mut err)
                        .await;
                }
                ("generate-account-number", Some(opt)) => {
                    call_result = self
                        ._accounts_generate_account_number(opt, dry_run, &mut err)
                        .await;
                }
                ("get", Some(opt)) => {
                ...
                ("get-notifications", Some(opt)) => {
                    call_result = self
                        ._accounts_get_notifications(opt, dry_run, &mut err)
                        .await;
                }
                ...
                ("list-recommend-google-locations", Some(opt)) => {
                    call_result = self
                        ._accounts_list_recommend_google_locations(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-admins-create", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_admins_create(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-admins-delete", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_admins_delete(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-admins-list", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_admins_list(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-admins-patch", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_admins_patch(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-associate", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_associate(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-batch-get", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_batch_get(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-batch-get-reviews", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_batch_get_reviews(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-clear-association", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_clear_association(opt, dry_run, &mut err)
                        .await;
                }
                ...
                ("locations-delete", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_delete(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-fetch-verification-options", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_fetch_verification_options(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-find-matches", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_find_matches(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-followers-get-metadata", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_followers_get_metadata(opt, dry_run, &mut err)
                        .await;
                }
                ...
                ("locations-get-google-updated", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_get_google_updated(opt, dry_run, &mut err)
                        .await;
                }
                ...
                ("locations-local-posts-create", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_local_posts_create(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-local-posts-delete", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_local_posts_delete(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-local-posts-get", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_local_posts_get(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-local-posts-list", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_local_posts_list(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-local-posts-patch", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_local_posts_patch(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-local-posts-report-insights", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_local_posts_report_insights(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-media-create", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_media_create(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-media-customers-get", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_media_customers_get(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-media-customers-list", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_media_customers_list(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-media-delete", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_media_delete(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-media-get", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_media_get(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-media-list", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_media_list(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-media-patch", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_media_patch(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-media-start-upload", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_media_start_upload(opt, dry_run, &mut err)
                        .await;
                }
                ...
                ("locations-questions-answers-delete", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_questions_answers_delete(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-questions-answers-list", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_questions_answers_list(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-questions-answers-upsert", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_questions_answers_upsert(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-questions-create", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_questions_create(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-questions-delete", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_questions_delete(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-questions-list", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_questions_list(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-questions-patch", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_questions_patch(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-report-insights", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_report_insights(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-reviews-delete-reply", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_reviews_delete_reply(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-reviews-get", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_reviews_get(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-reviews-list", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_reviews_list(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-reviews-update-reply", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_reviews_update_reply(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-transfer", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_transfer(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-verifications-complete", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_verifications_complete(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-verifications-list", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_verifications_list(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-verify", Some(opt)) => {
                    call_result = self
                        ._accounts_locations_verify(opt, dry_run, &mut err)
                        .await;
                }
                ("update", Some(opt)) => {
                ...
                ("update-notifications", Some(opt)) => {
                    call_result = self
                        ._accounts_update_notifications(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "mybusiness4-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/mybusiness4", config_dir))
        .build()
        .await
        .unwrap();
