...
use google_ondemandscanning1::{api, Error, oauth2, client::chrono, FieldMask};
    ...
    S::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    async fn _projects_locations_operations_wait(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.projects().locations_operations_wait(opt.value_of("name").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "timeout" => {
                    call = call.timeout(        value.map(|v| arg_from_str(v, err, "timeout", "google-duration")).unwrap_or(chrono::Duration::seconds(0)));
                },
                _ => {
                    let mut found = false;
                    for param in &self.gp {
    ...
    async fn _projects_locations_scans_analyze_packages(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    "include-osv-data" => Some(("includeOsvData", JsonTypeInfo { jtype: JsonType::Boolean, ctype: ComplexType::Pod })),
                    "resource-uri" => Some(("resourceUri", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Pod })),
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["include-osv-data", "resource-uri"]);
        ...
        let mut request: api::AnalyzePackagesRequestV1 = json::value::from_value(object).unwrap();
        let mut call = self.hub.projects().locations_scans_analyze_packages(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
    ...
    async fn _projects_locations_scans_vulnerabilities_list(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.projects().locations_scans_vulnerabilities_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "page-token" => {
            ...
            ("projects", Some(opt)) => {
                match opt.subcommand() {
                    ...
                    ("locations-scans-analyze-packages", Some(opt)) => {
                        call_result = self._projects_locations_scans_analyze_packages(opt, dry_run, &mut err).await;
                    },
                    ("locations-scans-vulnerabilities-list", Some(opt)) => {
                        call_result = self._projects_locations_scans_vulnerabilities_list(opt, dry_run, &mut err).await;
                    },
                    _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: S) -> Result<Engine<'n, S>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "ondemandscanning1-secret.json",
        ...
        let auth = oauth2::InstalledFlowAuthenticator::with_client(
            ...
            client.clone(),
        ).persist_tokens_to_disk(format!("{}/ondemandscanning1", config_dir)).build().await.unwrap();
