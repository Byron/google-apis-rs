...
impl Engine {
    ...
    fn _budget_patch(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::Budget = Default::default();
    ...
    fn _creatives_insert(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::Creative = Default::default();
        ...
        for kvarg in self.opt.arg_kv.iter() {
            ...
            match &field_name.to_string()[..] {
                ...
                "filtering-reasons.date" => {
                        ...
                        request.filtering_reasons.as_mut().unwrap().date = value.unwrap_or("").to_string();
                    },
    ...
    fn _pretargeting_config_insert(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::PretargetingConfig = Default::default();
    ...
    fn new(opt: Options) -> Result<Engine, InvalidOptionsError> {
        ...
        let engine = Engine {
            opt: opt,
            hub: api::AdExchangeBuyer::new(hyper::Client::new(), auth),
        };
