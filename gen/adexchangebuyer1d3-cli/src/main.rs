...
use google_adexchangebuyer1d3::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _billing_info_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let account_id: i32 = arg_from_str(
    ...
    async fn _creatives_get(
        ...
        let mut call = self
            .hub
            .creatives()
            .get(account_id, opt.value_of("buyer-creative-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _creatives_insert(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "native-ad.app-icon.height" => Some((
                ...
                "native-ad.click-tracking-url" => Some((
                ...
                "native-ad.impression-tracking-url" => Some((
    ...
    async fn _creatives_list(
            ...
            .iter()
        {
            ...
            match key {
                ...
                "account-id" => {
                    call = call.add_account_id(
                        value
                            .map(|v| arg_from_str(v, err, "account-id", "int32"))
                            .unwrap_or(-0),
                    );
                }
                _ => {
    ...
    async fn _performance_report_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _pretargeting_config_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _pretargeting_config_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _pretargeting_config_insert(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "excluded-geo-criteria-ids" => Some((
                ...
                "mobile-operating-system-versions" => Some((
                ...
                "supported-creative-attributes" => Some((
    ...
    async fn _pretargeting_config_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _pretargeting_config_patch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _pretargeting_config_update(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ...
            ("billing-info", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("billing-info".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("budget", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("budget".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("creatives", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("creatives".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("direct-deals", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._direct_deals_get(opt, dry_run, &mut err).await;
                }
                ("list", Some(opt)) => {
                    call_result = self._direct_deals_list(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("direct-deals".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("performance-report", Some(opt)) => match opt.subcommand() {
                ("list", Some(opt)) => {
            ...
            ("pretargeting-config", Some(opt)) => match opt.subcommand() {
                ("delete", Some(opt)) => {
                    call_result = self
                        ._pretargeting_config_delete(opt, dry_run, &mut err)
                        .await;
                }
                ("get", Some(opt)) => {
                ...
                ("insert", Some(opt)) => {
                    call_result = self
                        ._pretargeting_config_insert(opt, dry_run, &mut err)
                        .await;
                }
                ("list", Some(opt)) => {
                ...
                ("patch", Some(opt)) => {
                    call_result = self
                        ._pretargeting_config_patch(opt, dry_run, &mut err)
                        .await;
                }
                ("update", Some(opt)) => {
                    call_result = self
                        ._pretargeting_config_update(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues.push(CLIError::MissingMethodError(
                        "pretargeting-config".to_string(),
                    ));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "adexchangebuyer1d3-secret.json",
