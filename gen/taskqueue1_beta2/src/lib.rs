...
//! This documentation was generated from *taskqueue* crate version *0.1.6+20141111*, where *20141111* is the exact revision of the *taskqueue:v1beta2* schema built by the [mako](http://www.makotemplates.org/) code generator *v0.1.6*.
//! 
...
impl<'a, C, A> TaskqueueGetCall<'a, C, A> where C: BorrowMut<hyper::Client>, A: oauth2::GetToken {
    ...
    pub fn doit(mut self) -> Result<(hyper::client::Response, TaskQueue)> {
        ...
        }
        {
            ...
            for param_name in ["project", "taskqueue"].iter() {
...
impl<'a, C, A> TaskDeleteCall<'a, C, A> where C: BorrowMut<hyper::Client>, A: oauth2::GetToken {
    ...
    pub fn doit(mut self) -> Result<hyper::client::Response> {
        ...
        }
        {
            ...
            for param_name in ["project", "taskqueue", "task"].iter() {
                for (index, &(ref name, _)) in params.iter().rev().enumerate() {
                    if name == param_name {
                        indices_for_removal.push(params.len() - index - 1);
                        break;
