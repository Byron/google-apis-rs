...
impl<C> common::Hub for Taskqueue<C> {}

impl<'a, C> Taskqueue<C> {
    pub fn new<A: 'static + common::GetToken>(client: common::Client<C>, auth: A) -> Taskqueue<C> {
        Taskqueue {
            client,
            auth: Box::new(auth),
...
impl common::Resource for TaskQueue {}
impl common::ResponseResult for TaskQueue {}
...
impl common::ResponseResult for Tasks2 {}
...
impl common::NestedType for TaskQueueAcl {}
impl common::Part for TaskQueueAcl {}
...
impl common::NestedType for TaskQueueStats {}
impl common::Part for TaskQueueStats {}
...
impl<'a, C> common::MethodsBuilder for TaskqueueMethods<'a, C> {}
...
impl<'a, C> common::CallBuilder for TaskqueueGetCall<'a, C> {}

impl<'a, C> TaskqueueGetCall<'a, C>
where
    C: common::Connector,
{
    /// Perform the operation you have build so far.
    pub async fn doit(mut self) -> common::Result<(common::Response, TaskQueue)> {
        use std::borrow::Cow;
        ...
        for &(find_this, param_name) in
            [("{project}", "project"), ("{taskqueue}", "taskqueue")].iter()
        {
    ...
    pub fn delegate(mut self, new_value: &'a mut dyn common::Delegate) -> TaskqueueGetCall<'a, C> {
    ...
    pub fn add_scope<St>(mut self, scope: St) -> TaskqueueGetCall<'a, C>
    where
        St: AsRef<str>,
    {
    ...
    pub fn add_scopes<I, St>(mut self, scopes: I) -> TaskqueueGetCall<'a, C>
    where
    ...
    /// for details).
    pub fn clear_scopes(mut self) -> TaskqueueGetCall<'a, C> {
        self._scopes.clear();
        self
    }
}
...
impl<'a, C> common::CallBuilder for TaskLeaseCall<'a, C> {}

impl<'a, C> TaskLeaseCall<'a, C>
where
    C: common::Connector,
{
    /// Perform the operation you have build so far.
    ...
    ///
    /// Sets the *delegate* property to the given value.
    pub fn delegate(mut self, new_value: &'a mut dyn common::Delegate) -> TaskLeaseCall<'a, C> {
        self._delegate = Some(new_value);
        self
    }
    ...
    /// encompass more rights than others. For example, for listing resources, a *read-only* scope will be
    /// sufficient, a read-write scope will do as well.
    pub fn add_scope<St>(mut self, scope: St) -> TaskLeaseCall<'a, C>
    where
        St: AsRef<str>,
    {
        self._scopes.insert(String::from(scope.as_ref()));
    ...
    ///
    /// See [`Self::add_scope()`] for details.
    pub fn add_scopes<I, St>(mut self, scopes: I) -> TaskLeaseCall<'a, C>
    where
        I: IntoIterator<Item = St>,
        St: AsRef<str>,
    {
    ...
    /// In this case, you have to specify your API-key using the `key` parameter (see [`Self::param()`]
    /// for details).
    pub fn clear_scopes(mut self) -> TaskLeaseCall<'a, C> {
        self._scopes.clear();
        self
    }
}
...
{
    /// Perform the operation you have build so far.
    pub async fn doit(mut self) -> common::Result<(common::Response, Tasks2)> {
        use std::borrow::Cow;
        use std::io::{Read, Seek};
