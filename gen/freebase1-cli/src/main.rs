...
impl<'n, 'a> Engine<'n, 'a> {
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                Vec::new() + &self.gp + &["lang", "kind", "name", "confidence", "prop", "limit"]
                                                            ));
                    }
                }
            }
        ...
        for parg in opt.values_of("v").unwrap_or(Vec::new()).iter() {
            ...
            match key {
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                Vec::new() + &self.gp + &["domain", "help", "query", "stemmed", "cursor", "prefixed", "exact", "mid", "encode", "type", "as-of-time", "scoring", "format", "spell", "with", "lang", "indent", "filter", "callback", "without", "limit", "output", "mql-output"]
                                                            ));
                    }
                }
            }
        ...
        match self.opt.subcommand() {
            ("methods", Some(opt)) => {
                match opt.subcommand() {
                    ("reconcile", Some(opt)) => {
                        call_result = self._methods_reconcile(opt, dry_run, &mut err);
                    },
                    ("search", Some(opt)) => {
                        call_result = self._methods_search(opt, dry_run, &mut err);
                    },
                    _ => {
    ...
    fn new(opt: ArgMatches<'a, 'n>) -> Result<Engine<'a, 'n>, InvalidOptionsError> {
        ...
        let engine = Engine {
            opt: opt,
            hub: api::Freebase::new(client, auth),
