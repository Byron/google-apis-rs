...
use google_analyticsreporting4::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _reports_batch_get(
        ...
        let mut request: api::GetReportsRequest = serde_json::value::from_value(object).unwrap();
    ...
    async fn _user_activity_search(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::SearchUserActivityRequest =
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ...
            ("user-activity", Some(opt)) => match opt.subcommand() {
                ("search", Some(opt)) => {
                    call_result = self._user_activity_search(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("user-activity".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "analyticsreporting4-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/analyticsreporting4", config_dir))
        .build()
        .await
        .unwrap();
