...
/// ```
#[derive(Clone)]
pub struct AdExperienceReport<C> {
...
impl<C> common::Hub for AdExperienceReport<C> {}

impl<'a, C> AdExperienceReport<C> {
    pub fn new<A: 'static + common::GetToken>(
        client: common::Client<C>,
        auth: A,
    ) -> AdExperienceReport<C> {
        AdExperienceReport {
            client,
            auth: Box::new(auth),
    ...
    pub fn violating_sites(&'a self) -> ViolatingSiteMethods<'a, C> {
...
pub struct PlatformSummary {
    ...
    pub enforcement_time: Option<chrono::DateTime<chrono::offset::Utc>>,
    ...
    pub last_change_time: Option<chrono::DateTime<chrono::offset::Utc>>,
...
impl common::Part for PlatformSummary {}
...
impl common::ResponseResult for SiteSummaryResponse {}
...
impl common::ResponseResult for ViolatingSitesResponse {}
...
impl<'a, C> SiteMethods<'a, C> {
    ...
    pub fn get(&self, name: &str) -> SiteGetCall<'a, C> {
        SiteGetCall {
            hub: self.hub,
...
/// # }
/// ```
pub struct ViolatingSiteMethods<'a, C>
where
    C: 'a,
{
    hub: &'a AdExperienceReport<C>,
...
impl<'a, C> common::MethodsBuilder for ViolatingSiteMethods<'a, C> {}

impl<'a, C> ViolatingSiteMethods<'a, C> {
    /// Create a builder to help you perform the following task:
    ///
    /// Lists sites that are failing in the Ad Experience Report on at least one platform.
    pub fn list(&self) -> ViolatingSiteListCall<'a, C> {
    ...
    C: common::Connector,
{
    ...
    pub async fn doit(mut self) -> common::Result<(common::Response, SiteSummaryResponse)> {
        use std::borrow::Cow;
    ...
    pub fn name(mut self, new_value: &str) -> SiteGetCall<'a, C> {
...
/// # }
/// ```
pub struct ViolatingSiteListCall<'a, C>
where
    C: 'a,
{
    hub: &'a AdExperienceReport<C>,
...
impl<'a, C> common::CallBuilder for ViolatingSiteListCall<'a, C> {}

impl<'a, C> ViolatingSiteListCall<'a, C>
where
    C: common::Connector,
{
    /// Perform the operation you have build so far.
    pub async fn doit(mut self) -> common::Result<(common::Response, ViolatingSitesResponse)> {
        use std::borrow::Cow;
        use std::io::{Read, Seek};
    ...
    /// * *uploadType* (query-string) - Legacy upload protocol for media (e.g. "media", "multipart").
    /// * *upload_protocol* (query-string) - Upload protocol for media (e.g. "raw", "multipart").
    pub fn param<T>(mut self, name: T, value: T) -> ViolatingSiteListCall<'a, C>
    where
        T: AsRef<str>,
    {
        self._additional_params
