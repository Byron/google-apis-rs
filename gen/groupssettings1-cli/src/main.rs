...
use google_groupssettings1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _groups_get(
        ...
        let mut call = self
            .hub
            .groups()
            .get(opt.value_of("group-unique-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _groups_patch(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "allow-google-communication" => Some((
                ...
                "custom-roles-enabled-for-settings-to-be-merged" => Some((
                ...
                "default-message-deny-notification-text" => Some((
                ...
                "enable-collaborative-inbox" => Some((
                ...
                "include-in-global-address-list" => Some((
                ...
                "members-can-post-as-the-group" => Some((
                ...
                "send-message-deny-notification" => Some((
                ...
                "who-can-enter-free-form-tags" => Some((
                ...
                "who-can-make-topics-sticky" => Some((
                ...
                "who-can-mark-favorite-reply-on-any-topic" => Some((
                ...
                "who-can-mark-favorite-reply-on-own-topic" => Some((
                ...
                "who-can-mark-no-response-needed" => Some((
                ...
                "who-can-modify-tags-and-categories" => Some((
                ...
                "who-can-post-announcements" => Some((
                ...
                "who-can-unmark-favorite-reply-on-any-topic" => Some((
        ...
        let mut request: api::Groups = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .groups()
            .patch(request, opt.value_of("group-unique-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _groups_update(
        ...
        let mut call = self
            .hub
            .groups()
            .update(request, opt.value_of("group-unique-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "groupssettings1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/groupssettings1", config_dir))
        .build()
        .await
        .unwrap();
