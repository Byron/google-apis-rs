...
/// A builder providing access to all methods supported on *customEvent* resources.
/// It is not used directly, but through the [`Dfareporting`] hub.
///
/// # Example
///
...
/// // like `batchinsert(...)`
/// // to build up your call.
/// let rb = hub.custom_events();
/// # }
/// ```
pub struct CustomEventMethods<'a, S>
    where S: 'a {

   pub(super) hub: &'a Dfareporting<S>,
}

impl<'a, S> client::MethodsBuilder for CustomEventMethods<'a, S> {}

impl<'a, S> CustomEventMethods<'a, S> {
    ...
    /// * `request` - No description provided.
    /// * `profileId` - User profile ID associated with this request.
    pub fn batchinsert(&self, request: CustomEventsBatchInsertRequest, profile_id: i64) -> CustomEventBatchinsertCall<'a, S> {
        CustomEventBatchinsertCall {
            hub: self.hub,
            _request: request,
            _profile_id: profile_id,
            _delegate: Default::default(),
