...
impl<'n> Engine<'n> {
    ...
    fn _projects_test_matrices_create(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    fn _projects_test_matrices_get(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    fn _test_environment_catalog_get(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    fn _doit(&self, dry_run: bool) -> Result<Result<(), DoitError>, Option<InvalidOptionsError>> {
        ...
        match self.opt.subcommand() {
            ...
            ("projects", Some(opt)) => {
                match opt.subcommand() {
                    ("test-matrices-cancel", Some(opt)) => {
                        call_result = self._projects_test_matrices_cancel(opt, dry_run, &mut err);
                    },
                    ("test-matrices-create", Some(opt)) => {
                        call_result = self._projects_test_matrices_create(opt, dry_run, &mut err);
                    },
                    ("test-matrices-get", Some(opt)) => {
                        call_result = self._projects_test_matrices_get(opt, dry_run, &mut err);
                    },
                    _ => {
            ...
            ("test-environment-catalog", Some(opt)) => {
                match opt.subcommand() {
                    ("get", Some(opt)) => {
                        call_result = self._test_environment_catalog_get(opt, dry_run, &mut err);
                    },
                    _ => {
    ...
    let arg_data = [
        ...
        - NOT_FOUND - if the Test Matrix does not exist"##),
            ...
            ("test-matrices-create",
                    Some(r##"Creates and runs a matrix of tests according to the given specifications.
        ...
        - INVALID_ARGUMENT - if the request is malformed or if the matrix expands
            ...
            ("test-matrices-get",
                    Some(r##"Checks the status of a test matrix.
        ...
        ("test-environment-catalog", "methods: 'get'", vec![
            ("get",
                    Some(r##"Gets the catalog of supported test environments.
