...
use google_doubleclickbidmanager1::{api, yup_oauth2, Error};
...
struct Engine<'n, C> {
    opt: ArgMatches<'n>,
    hub: api::DoubleClickBidManager<C>,
    gp: Vec<&'static str>,
...
{
    async fn _lineitems_downloadlineitems(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["file-spec", "filter-ids", "filter-type", "format"],
                    );
        ...
        let mut request: api::DownloadLineItemsRequest =
    ...
    async fn _lineitems_uploadlineitems(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion =
                        FieldCursor::did_you_mean(key, &vec!["dry-run", "format", "line-items"]);
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
                        value.map(|v| v.to_string()),
        ...
        let mut request: api::UploadLineItemsRequest =
            serde_json::value::from_value(object).unwrap();
    ...
    async fn _queries_createquery(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "metadata.google-cloud-storage-path-for-latest-report" => Some((
                ...
                "metadata.google-drive-path-for-latest-report" => Some((
                ...
                "metadata.latest-report-run-time-ms" => Some((
                ...
                "metadata.send-notification" => Some((
                ...
                "metadata.share-email-address" => Some((
                ...
                "params.include-invite-data" => Some((
                ...
                "report-data-start-time-ms" => Some((
                ...
                "schedule.next-run-minute-of-day" => Some((
                ...
                "schedule.next-run-timezone-code" => Some((
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        ...
                        &vec![
                            ...
                            "format",
                            "frequency",
                            "google-cloud-storage-path-for-latest-report",
                            "google-drive-path-for-latest-report",
                            "group-bys",
                            "include-invite-data",
                            "kind",
                            "latest-report-run-time-ms",
                            "locale",
                            "metadata",
                            "metrics",
                            "next-run-minute-of-day",
        ...
        let mut request: api::Query = serde_json::value::from_value(object).unwrap();
    ...
    async fn _sdf_download(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["file-types", "filter-ids", "filter-type", "version"],
                    );
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
        ...
        let mut request: api::DownloadRequest = serde_json::value::from_value(object).unwrap();
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("lineitems", Some(opt)) => match opt.subcommand() {
                ("downloadlineitems", Some(opt)) => {
                    call_result = self
                        ._lineitems_downloadlineitems(opt, dry_run, &mut err)
                        .await;
                }
                ("uploadlineitems", Some(opt)) => {
                    call_result = self
                        ._lineitems_uploadlineitems(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("lineitems".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("queries", Some(opt)) => match opt.subcommand() {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("queries".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ...
            ("sdf", Some(opt)) => match opt.subcommand() {
                ("download", Some(opt)) => {
                    call_result = self._sdf_download(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("sdf".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "doubleclickbidmanager1-secret.json",
