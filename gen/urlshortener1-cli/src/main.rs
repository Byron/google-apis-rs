...
use google_urlshortener1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _url_insert(
        ...
        let mut request: api::Url = serde_json::value::from_value(object).unwrap();
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "urlshortener1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/urlshortener1", config_dir))
        .build()
        .await
        .unwrap();
