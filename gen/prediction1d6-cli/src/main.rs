...
impl Engine {
    fn _hostedmodels_predict(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::Input = Default::default();
    ...
    fn _trainedmodels_analyze(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
        let mut call = self.hub.trainedmodels().analyze(&self.opt.arg_project, &self.opt.arg_id);
        for parg in self.opt.arg_v.iter() {
            let (key, value) = parse_kv_arg(&*parg, err);
            match key {
                "alt"
    ...
    fn _trainedmodels_delete(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
        let mut call = self.hub.trainedmodels().delete(&self.opt.arg_project, &self.opt.arg_id);
        for parg in self.opt.arg_v.iter() {
            let (key, value) = parse_kv_arg(&*parg, err);
            match key {
                "alt"
    ...
    fn _trainedmodels_get(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
        let mut call = self.hub.trainedmodels().get(&self.opt.arg_project, &self.opt.arg_id);
        for parg in self.opt.arg_v.iter() {
            let (key, value) = parse_kv_arg(&*parg, err);
            match key {
                "alt"
    ...
    fn _trainedmodels_insert(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::Insert = Default::default();
    ...
    fn _trainedmodels_list(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
        let mut call = self.hub.trainedmodels().list(&self.opt.arg_project);
        for parg in self.opt.arg_v.iter() {
            let (key, value) = parse_kv_arg(&*parg, err);
            match key {
                "page-token" => {
    ...
    fn _trainedmodels_update(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::Update = Default::default();
    ...
    fn _doit(&self, dry_run: bool) -> (Option<api::Error>, Option<InvalidOptionsError>) {
        ...
        if self.opt.cmd_hostedmodels {
            if self.opt.cmd_predict {
                call_result = self._hostedmodels_predict(dry_run, &mut err);
            } else {
                unreachable!();
            }
        } else if self.opt.cmd_trainedmodels {
            if self.opt.cmd_analyze {
                call_result = self._trainedmodels_analyze(dry_run, &mut err);
            } else if self.opt.cmd_delete {
                call_result = self._trainedmodels_delete(dry_run, &mut err);
            } else if self.opt.cmd_get {
                call_result = self._trainedmodels_get(dry_run, &mut err);
            } else if self.opt.cmd_insert {
                call_result = self._trainedmodels_insert(dry_run, &mut err);
            } else if self.opt.cmd_list {
                call_result = self._trainedmodels_list(dry_run, &mut err);
            } else if self.opt.cmd_predict {
                call_result = self._trainedmodels_predict(dry_run, &mut err);
            } else if self.opt.cmd_update {
                call_result = self._trainedmodels_update(dry_run, &mut err);
            } else {
                unreachable!();
            }
        } else {
    ...
    fn new(opt: Options) -> Result<Engine, InvalidOptionsError> {
        ...
        let engine = Engine {
            opt: opt,
            hub: api::Prediction::new(hyper::Client::new(), auth),
        };
