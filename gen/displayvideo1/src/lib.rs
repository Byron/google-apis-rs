...
//! This documentation was generated from *Display Video* crate version *8.0.0+20251027*, where *20251027* is the exact revision of the *displayvideo:v1* schema built by the [mako](http://www.makotemplates.org/) code generator *v8.0.0*.
//!
...
//! let r = hub.advertisers().campaigns_list_assigned_targeting_options(...).doit().await
...
//! let r = hub.advertisers().insertion_orders_targeting_types_assigned_targeting_options_create(...).doit().await
//! let r = hub.advertisers().insertion_orders_targeting_types_assigned_targeting_options_delete(...).doit().await
...
//! let r = hub.advertisers().insertion_orders_list_assigned_targeting_options(...).doit().await
