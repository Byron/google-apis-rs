...
use google_gameservices1::{api, yup_oauth2, Error};
...
{
    async fn _projects_locations_game_server_deployments_get_iam_policy(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
    ...
    async fn _projects_locations_game_server_deployments_set_iam_policy(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_game_server_deployments_set_iam_policy(
                request,
                opt.value_of("resource").unwrap_or(""),
            );
        for parg in opt
    ...
    async fn _projects_locations_game_server_deployments_test_iam_permissions(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_game_server_deployments_test_iam_permissions(
                request,
                opt.value_of("resource").unwrap_or(""),
            );
        for parg in opt
        ...
        match self.opt.subcommand() {
            ("projects", Some(opt)) => match opt.subcommand() {
                ("locations-game-server-deployments-get-iam-policy", Some(opt)) => {
                    call_result = self
                        ._projects_locations_game_server_deployments_get_iam_policy(
                            opt, dry_run, &mut err,
                        )
                        .await;
                }
                ("locations-game-server-deployments-set-iam-policy", Some(opt)) => {
                    call_result = self
                        ._projects_locations_game_server_deployments_set_iam_policy(
                            opt, dry_run, &mut err,
                        )
                        .await;
                }
                ("locations-game-server-deployments-test-iam-permissions", Some(opt)) => {
                    call_result = self
                        ._projects_locations_game_server_deployments_test_iam_permissions(
                            opt, dry_run, &mut err,
                        )
                        .await;
                }
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "gameservices1-secret.json",
