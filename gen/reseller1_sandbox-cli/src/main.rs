...
use google_reseller1_sandbox::{api, Error, oauth2, client::chrono, FieldMask};
    ...
    S::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    async fn _subscriptions_change_plan(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        let mut request: api::ChangePlanRequest = json::value::from_value(object).unwrap();
    ...
    async fn _subscriptions_change_renewal_settings(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        let mut request: api::RenewalSettings = json::value::from_value(object).unwrap();
    ...
    async fn _subscriptions_change_seats(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        let mut request: api::Seats = json::value::from_value(object).unwrap();
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["customer-auth-token", "customer-id", "customer-name-prefix", "max-results", "page-token"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
