...
impl Engine {
    ...
    fn _customers_insert(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::Customer = Default::default();
        ...
        for kvarg in self.opt.arg_kv.iter() {
            ...
            match &field_name.to_string()[..] {
                ...
                "postal-address.kind" => {
                        ...
                        request.postal_address.as_mut().unwrap().kind = value.unwrap_or("").to_string();
                    },
                "postal-address.organization-name" => {
                        request_postal_address_init(&mut request);
                        request.postal_address.as_mut().unwrap().organization_name = value.unwrap_or("").to_string();
                    },
                "postal-address.country-code" => {
                        request_postal_address_init(&mut request);
                        request.postal_address.as_mut().unwrap().country_code = value.unwrap_or("").to_string();
                    },
                "postal-address.locality" => {
                        request_postal_address_init(&mut request);
                        request.postal_address.as_mut().unwrap().locality = value.unwrap_or("").to_string();
                    },
                "postal-address.region" => {
                        request_postal_address_init(&mut request);
                        request.postal_address.as_mut().unwrap().region = value.unwrap_or("").to_string();
                    },
                "postal-address.address-line2" => {
                        request_postal_address_init(&mut request);
                        request.postal_address.as_mut().unwrap().address_line2 = value.unwrap_or("").to_string();
                    },
                "postal-address.address-line3" => {
                        request_postal_address_init(&mut request);
                        request.postal_address.as_mut().unwrap().address_line3 = value.unwrap_or("").to_string();
                    },
                "postal-address.contact-name" => {
                        request_postal_address_init(&mut request);
                        request.postal_address.as_mut().unwrap().contact_name = value.unwrap_or("").to_string();
                    },
                "postal-address.address-line1" => {
                        request_postal_address_init(&mut request);
                        request.postal_address.as_mut().unwrap().address_line1 = value.unwrap_or("").to_string();
                    },
                "postal-address.postal-code" => {
                        request_postal_address_init(&mut request);
                        request.postal_address.as_mut().unwrap().postal_code = value.unwrap_or("").to_string();
                    },
                "customer-id" => {
                        request_postal_address_init(&mut request);
    ...
    fn _subscriptions_change_plan(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::ChangePlanRequest = Default::default();
    ...
    fn _subscriptions_change_renewal_settings(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::RenewalSettings = Default::default();
    ...
    fn _subscriptions_change_seats(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::Seats = Default::default();
        ...
        for kvarg in self.opt.arg_kv.iter() {
            ...
            match &field_name.to_string()[..] {
                ...
                "trial-settings.trial-end-time" => {
                        ...
                        request.trial_settings.as_mut().unwrap().trial_end_time = value.unwrap_or("").to_string();
                    },
                "trial-settings.is-in-trial" => {
                        request_trial_settings_init(&mut request);
                        request.trial_settings.as_mut().unwrap().is_in_trial = arg_from_str(value.unwrap_or("false"), err, "trial-settings.is-in-trial", "boolean");
                    },
                "transfer-info.transferability-expiration-time" => {
                        request_transfer_info_init(&mut request);
                        request.transfer_info.as_mut().unwrap().transferability_expiration_time = value.unwrap_or("").to_string();
                    },
                "transfer-info.minimum-transferable-seats" => {
                        request_transfer_info_init(&mut request);
                        request.transfer_info.as_mut().unwrap().minimum_transferable_seats = arg_from_str(value.unwrap_or("-0"), err, "transfer-info.minimum-transferable-seats", "integer");
                    },
                "resource-ui-url" => {
                        request_transfer_info_init(&mut request);
                        request.resource_ui_url = Some(value.unwrap_or("").to_string());
                ...
                "plan.plan-name" => {
                        ...
                        request.plan.as_mut().unwrap().plan_name = value.unwrap_or("").to_string();
                    },
                "plan.commitment-interval.end-time" => {
                        request_plan_init(&mut request);
                        request.plan.as_mut().unwrap().commitment_interval.end_time = value.unwrap_or("").to_string();
                    },
                "plan.commitment-interval.start-time" => {
                        request_plan_init(&mut request);
                        request.plan.as_mut().unwrap().commitment_interval.start_time = value.unwrap_or("").to_string();
                    },
                "plan.is-commitment-plan" => {
                        request_plan_init(&mut request);
                        request.plan.as_mut().unwrap().is_commitment_plan = arg_from_str(value.unwrap_or("false"), err, "plan.is-commitment-plan", "boolean");
                    },
                "purchase-order-id" => {
                        request_plan_init(&mut request);
                        request.purchase_order_id = Some(value.unwrap_or("").to_string());
    ...
    fn new(opt: Options) -> Result<Engine, InvalidOptionsError> {
        ...
        let engine = Engine {
            opt: opt,
            hub: api::Reseller::new(hyper::Client::new(), auth),
        };
