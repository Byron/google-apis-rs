...
use google_mirror1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _accounts_insert(
        ...
        let mut call = self.hub.accounts().insert(
            request,
            opt.value_of("user-token").unwrap_or(""),
            opt.value_of("account-type").unwrap_or(""),
            opt.value_of("account-name").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
    ...
    async fn _contacts_insert(
        ...
        let mut request: api::Contact = serde_json::value::from_value(object).unwrap();
    ...
    async fn _subscriptions_insert(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "notification.verify-token" => Some((
    ...
    async fn _subscriptions_update(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _timeline_attachments_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.timeline().attachments_delete(
    ...
    async fn _timeline_attachments_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self.hub.timeline().attachments_get(
            opt.value_of("item-id").unwrap_or(""),
            opt.value_of("attachment-id").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _timeline_attachments_insert(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .timeline()
            .attachments_insert(opt.value_of("item-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _timeline_attachments_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .timeline()
            .attachments_list(opt.value_of("item-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _timeline_insert(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "creator.sharing-features" => Some((
                    "creator.sharingFeatures",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Vec,
                    },
                ...
                "creator.speakable-name" => Some((
                    "creator.speakableName",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                ...
                "notification.delivery-time" => Some((
        ...
        let mut request: api::TimelineItem = serde_json::value::from_value(object).unwrap();
    ...
    async fn _timeline_list(
            ...
            .iter()
        {
            ...
            match key {
                ...
                "pinned-only" => {
                    call = call.pinned_only(
                        value
                            .map(|v| arg_from_str(v, err, "pinned-only", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "page-token" => {
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ...
            ("contacts", Some(opt)) => match opt.subcommand() {
                ("delete", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("contacts".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ...
            ("timeline", Some(opt)) => match opt.subcommand() {
                ("attachments-delete", Some(opt)) => {
                    call_result = self
                        ._timeline_attachments_delete(opt, dry_run, &mut err)
                        .await;
                }
                ...
                ("attachments-insert", Some(opt)) => {
                    call_result = self
                        ._timeline_attachments_insert(opt, dry_run, &mut err)
                        .await;
                }
                ("attachments-list", Some(opt)) => {
                    call_result = self
                        ._timeline_attachments_list(opt, dry_run, &mut err)
                        .await;
                }
                ("delete", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("timeline".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "mirror1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/mirror1", config_dir))
        .build()
        .await
        .unwrap();
