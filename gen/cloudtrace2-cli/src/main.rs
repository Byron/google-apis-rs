...
impl<'n> Engine<'n> {
    async fn _projects_traces_batch_write(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    async fn _projects_traces_spans_create_span(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
            ...
            ("projects", Some(opt)) => {
                match opt.subcommand() {
                    ("traces-batch-write", Some(opt)) => {
                        call_result = self._projects_traces_batch_write(opt, dry_run, &mut err).await;
                    },
                    ("traces-spans-create-span", Some(opt)) => {
                        call_result = self._projects_traces_spans_create_span(opt, dry_run, &mut err).await;
                    },
                    _ => {
...
async fn main() {
    ...
    let arg_data = [
        ("projects", "methods: 'traces-batch-write' and 'traces-spans-create-span'", vec![
            ("traces-batch-write",
                    Some(r##"Sends new spans to new or existing traces. You cannot update existing spans."##),
