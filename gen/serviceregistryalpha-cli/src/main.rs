...
use google_serviceregistryalpha::{api, yup_oauth2, Error};
...
{
    async fn _endpoints_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.endpoints().delete(
    ...
    async fn _endpoints_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.endpoints().get(
            opt.value_of("project").unwrap_or(""),
            opt.value_of("endpoint").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _endpoints_patch(
        ...
        let mut request: api::Endpoint = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.endpoints().patch(
            request,
            opt.value_of("project").unwrap_or(""),
            opt.value_of("endpoint").unwrap_or(""),
        );
    ...
    async fn _endpoints_update(
        ...
        let mut request: api::Endpoint = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.endpoints().update(
            request,
            opt.value_of("project").unwrap_or(""),
            opt.value_of("endpoint").unwrap_or(""),
        );
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("endpoints", Some(opt)) => match opt.subcommand() {
                ("delete", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("endpoints".to_string()));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "serviceregistryalpha-secret.json",
