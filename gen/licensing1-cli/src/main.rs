...
extern crate hyper;
extern crate mime;
extern crate google_licensing1 as api;
...
struct Engine {
    opt: Options,
    hub: api::Licensing<hyper::Client, Authenticator<DefaultAuthenticatorDelegate, JsonTokenStorage, hyper::Client>>,
}


impl Engine {
    fn _license_assignments_delete(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
        let mut call = self.hub.license_assignments().delete(&self.opt.arg_product_id, &self.opt.arg_sku_id, &self.opt.arg_user_id);
    ...
    fn _license_assignments_get(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
        let mut call = self.hub.license_assignments().get(&self.opt.arg_product_id, &self.opt.arg_sku_id, &self.opt.arg_user_id);
        for parg in self.opt.arg_v.iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "alt"
    ...
    fn _license_assignments_list_for_product(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
        let mut call = self.hub.license_assignments().list_for_product(&self.opt.arg_product_id, &self.opt.arg_customer_id);
        for parg in self.opt.arg_v.iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "page-token" => {
    ...
    fn _license_assignments_list_for_product_and_sku(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
        let mut call = self.hub.license_assignments().list_for_product_and_sku(&self.opt.arg_product_id, &self.opt.arg_sku_id, &self.opt.arg_customer_id);
        for parg in self.opt.arg_v.iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "page-token" => {
    ...
    fn _doit(&self, dry_run: bool) -> (Option<api::Error>, Option<InvalidOptionsError>) {
        ...
        if self.opt.cmd_license_assignments {
            if self.opt.cmd_delete {
                call_result = self._license_assignments_delete(dry_run, &mut err);
            } else if self.opt.cmd_get {
                call_result = self._license_assignments_get(dry_run, &mut err);
            } else if self.opt.cmd_insert {
                call_result = self._license_assignments_insert(dry_run, &mut err);
            } else if self.opt.cmd_list_for_product {
                call_result = self._license_assignments_list_for_product(dry_run, &mut err);
            } else if self.opt.cmd_list_for_product_and_sku {
                call_result = self._license_assignments_list_for_product_and_sku(dry_run, &mut err);
            } else if self.opt.cmd_patch {
                call_result = self._license_assignments_patch(dry_run, &mut err);
            } else if self.opt.cmd_update {
                call_result = self._license_assignments_update(dry_run, &mut err);
            } else {
                unreachable!();
            }
        } else {
    ...
    fn new(opt: Options) -> Result<Engine, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match cmn::application_secret_from_directory(&config_dir, "licensing1-secret.json", 
