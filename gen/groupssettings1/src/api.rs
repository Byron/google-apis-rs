...
impl<C> common::Hub for Groupssettings<C> {}
...
impl common::RequestValue for Groups {}
    ...
    C: common::Connector,
{
    ...
    pub async fn doit(mut self) -> common::Result<(common::Response, Groups)> {
        use std::borrow::Cow;
        ...
        if self._scopes.is_empty() {
            self._scopes
                .insert(Scope::AppGroupSetting.as_ref().to_string());
        }
...
impl<'a, C> common::CallBuilder for GroupUpdateCall<'a, C> {}

impl<'a, C> GroupUpdateCall<'a, C>
where
    C: common::Connector,
{
    /// Perform the operation you have build so far.
    ...
    ///
    /// Sets the *delegate* property to the given value.
    pub fn delegate(mut self, new_value: &'a mut dyn common::Delegate) -> GroupUpdateCall<'a, C> {
        self._delegate = Some(new_value);
        self
    }
    ...
    /// encompass more rights than others. For example, for listing resources, a *read-only* scope will be
    /// sufficient, a read-write scope will do as well.
    pub fn add_scope<St>(mut self, scope: St) -> GroupUpdateCall<'a, C>
    where
        St: AsRef<str>,
    {
        self._scopes.insert(String::from(scope.as_ref()));
    ...
    ///
    /// See [`Self::add_scope()`] for details.
    pub fn add_scopes<I, St>(mut self, scopes: I) -> GroupUpdateCall<'a, C>
    where
        I: IntoIterator<Item = St>,
        St: AsRef<str>,
    {
    ...
    /// In this case, you have to specify your API-key using the `key` parameter (see [`Self::param()`]
    /// for details).
    pub fn clear_scopes(mut self) -> GroupUpdateCall<'a, C> {
        self._scopes.clear();
        self
    }
}
