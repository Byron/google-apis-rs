...
use google_appsactivity1::{api, Error};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::Appsactivity<hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>
    >,
    gp: Vec<&'static str>,
...
impl<'n> Engine<'n> {
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "appsactivity1-secret.json",
        ...
        ).persist_tokens_to_disk(format!("{}/appsactivity1", config_dir)).build().await.unwrap();

        let client = hyper::Client::builder().build(hyper_rustls::HttpsConnector::with_native_roots());
        let engine = Engine {
            opt: opt,
            hub: api::Appsactivity::new(client, auth),
