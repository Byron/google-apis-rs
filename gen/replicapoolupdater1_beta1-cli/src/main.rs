...
impl Engine {
    ...
    fn _rolling_updates_insert(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::RollingUpdate = Default::default();
        ...
        for kvarg in self.opt.arg_kv.iter() {
            ...
            match &field_name.to_string()[..] {
                ...
                "policy.instance-startup-timeout-sec" => {
                        ...
                        request.policy.as_mut().unwrap().instance_startup_timeout_sec = arg_from_str(value.unwrap_or("-0"), err, "policy.instance-startup-timeout-sec", "integer");
                    },
                "policy.max-num-concurrent-instances" => {
                        request_policy_init(&mut request);
                        request.policy.as_mut().unwrap().max_num_concurrent_instances = arg_from_str(value.unwrap_or("-0"), err, "policy.max-num-concurrent-instances", "integer");
                    },
                "policy.min-instance-update-time-sec" => {
                        request_policy_init(&mut request);
                        request.policy.as_mut().unwrap().min_instance_update_time_sec = arg_from_str(value.unwrap_or("-0"), err, "policy.min-instance-update-time-sec", "integer");
                    },
                "policy.sleep-after-instance-restart-sec" => {
                        request_policy_init(&mut request);
                        request.policy.as_mut().unwrap().sleep_after_instance_restart_sec = arg_from_str(value.unwrap_or("-0"), err, "policy.sleep-after-instance-restart-sec", "integer");
                    },
                "policy.max-num-failed-instances" => {
                        request_policy_init(&mut request);
                        request.policy.as_mut().unwrap().max_num_failed_instances = arg_from_str(value.unwrap_or("-0"), err, "policy.max-num-failed-instances", "integer");
                    },
                "policy.auto-pause-after-instances" => {
                        request_policy_init(&mut request);
                        request.policy.as_mut().unwrap().auto_pause_after_instances = arg_from_str(value.unwrap_or("-0"), err, "policy.auto-pause-after-instances", "integer");
                    },
                "progress" => {
                        request_policy_init(&mut request);
    ...
    fn new(opt: Options) -> Result<Engine, InvalidOptionsError> {
        ...
        let engine = Engine {
            opt: opt,
            hub: api::Replicapoolupdater::new(hyper::Client::new(), auth),
        };
