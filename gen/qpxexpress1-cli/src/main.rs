...
use google_qpxexpress1::{api, Error};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::QPXExpress<hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>
    >,
    gp: Vec<&'static str>,
...
impl<'n> Engine<'n> {
    async fn _trips_search(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    "request.passengers.infant-in-seat-count" => Some(("request.passengers.infantInSeatCount", JsonTypeInfo { jtype: JsonType::Int, ctype: ComplexType::Pod })),
        ...
        let mut request: api::TripsSearchRequest = json::value::from_value(object).unwrap();
        ...
        match self.opt.subcommand() {
            ("trips", Some(opt)) => {
                match opt.subcommand() {
                    ...
                    _ => {
                        err.issues.push(CLIError::MissingMethodError("trips".to_string()));
                        writeln!(io::stderr(), "{}\n", opt.usage()).ok();
                    }
                }
            },
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "qpxexpress1-secret.json",
