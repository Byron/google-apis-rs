...
use google_qpxexpress1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _trips_search(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "request.passengers.adult-count" => Some((
                ...
                "request.passengers.child-count" => Some((
                    "request.passengers.childCount",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "request.passengers.infant-in-lap-count" => Some((
                    "request.passengers.infantInLapCount",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "request.passengers.infant-in-seat-count" => Some((
                    "request.passengers.infantInSeatCount",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "request.passengers.kind" => Some((
                ...
                "request.passengers.senior-count" => Some((
                    "request.passengers.seniorCount",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "request.refundable" => Some((
                    "request.refundable",
                    JsonTypeInfo {
                ...
                "request.ticketing-country" => Some((
                    "request.ticketingCountry",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
        ...
        let mut request: api::TripsSearchRequest = serde_json::value::from_value(object).unwrap();
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("trips", Some(opt)) => match opt.subcommand() {
                ("search", Some(opt)) => {
                    call_result = self._trips_search(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("trips".to_string()));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "qpxexpress1-secret.json",
...
async fn main() {
    ...
    let arg_data = [(
        ...
        vec![(
            "search",
            Some(r##"Returns a list of flights."##),
            "Details at http://byron.github.io/google-apis-rs/google_qpxexpress1_cli/trips_search",
            vec![
                (
                    Some(r##"kv"##),
                    Some(r##"r"##),
