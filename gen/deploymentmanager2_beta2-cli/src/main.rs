...
use google_deploymentmanager2_beta2::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["create-policy", "delete-policy", "update-policy"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "deploymentmanager2-beta2-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/deploymentmanager2-beta2", config_dir))
        .build()
        .await
        .unwrap();
