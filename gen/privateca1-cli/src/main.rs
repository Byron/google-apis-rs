...
use google_privateca1::{api, Error, oauth2};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::CertificateAuthorityService,
    gp: Vec<&'static str>,
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                ...
                "ignore-active-certificates" => {
                    call = call.ignore_active_certificates(arg_from_str(value.unwrap_or("false"), err, "ignore-active-certificates", "boolean"));
                },
                _ => {
                    let mut found = false;
                    for param in &self.gp {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["ignore-active-certificates", "request-id"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
