...
use google_playcustomapp1::{api, Error, oauth2, client::chrono, FieldMask};
...
{
    async fn _accounts_custom_apps_create(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
            ...
            ("accounts", Some(opt)) => {
                match opt.subcommand() {
                    ("custom-apps-create", Some(opt)) => {
                        call_result = self._accounts_custom_apps_create(opt, dry_run, &mut err).await;
                    },
                    _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: S) -> Result<Engine<'n, S>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "playcustomapp1-secret.json",
        ...
        let auth = oauth2::InstalledFlowAuthenticator::with_client(
            ...
            client.clone(),
        ).persist_tokens_to_disk(format!("{}/playcustomapp1", config_dir)).build().await.unwrap();
