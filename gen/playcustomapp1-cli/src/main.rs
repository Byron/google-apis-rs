...
extern crate mime;
extern crate strsim;
extern crate google_playcustomapp1 as api;
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::Playcustomapp<hyper::Client, Authenticator<DefaultAuthenticatorDelegate, JsonTokenStorage, hyper::Client>>,
    gp: Vec<&'static str>,
...
impl<'n> Engine<'n> {
    fn _accounts_custom_apps_create(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut request: api::CustomApp = json::value::from_value(object).unwrap();
        let mut call = self.hub.accounts().custom_apps_create(request, opt.value_of("account").unwrap_or(""));
            ...
            ("accounts", Some(opt)) => {
                match opt.subcommand() {
                    ("custom-apps-create", Some(opt)) => {
                        call_result = self._accounts_custom_apps_create(opt, dry_run, &mut err);
                    },
                    _ => {
    ...
    fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match cmn::application_secret_from_directory(&config_dir, "playcustomapp1-secret.json",
