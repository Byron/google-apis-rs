...
use google_recommender1_beta1::{api, Error, oauth2};
...
impl<'n> Engine<'n> {
    ...
    async fn _organizations_locations_insight_types_config(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut request: api::GoogleCloudRecommenderV1beta1InsightTypeConfig = json::value::from_value(object).unwrap();
        let mut call = self.hub.organizations().locations_insight_types_config(request, opt.value_of("name").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "validate-only" => {
    ...
    async fn _organizations_locations_recommenders_config(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut request: api::GoogleCloudRecommenderV1beta1RecommenderConfig = json::value::from_value(object).unwrap();
        let mut call = self.hub.organizations().locations_recommenders_config(request, opt.value_of("name").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "validate-only" => {
    ...
    async fn _doit(&self, dry_run: bool) -> Result<Result<(), DoitError>, Option<InvalidOptionsError>> {
        ...
        match self.opt.subcommand() {
            ...
            ("organizations", Some(opt)) => {
                match opt.subcommand() {
                    ("locations-insight-types-config", Some(opt)) => {
                        call_result = self._organizations_locations_insight_types_config(opt, dry_run, &mut err).await;
                    },
                    ...
                    ("locations-recommenders-config", Some(opt)) => {
                        call_result = self._organizations_locations_recommenders_config(opt, dry_run, &mut err).await;
                    },
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "recommender1-beta1-secret.json",
        ...
        ).persist_tokens_to_disk(format!("{}/recommender1-beta1", config_dir)).build().await.unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ...
        ("organizations", "methods: 'locations-insight-types-config', 'locations-insight-types-get-config', 'locations-insight-types-insights-get', 'locations-insight-types-insights-list', 'locations-insight-types-insights-mark-accepted', 'locations-recommenders-config', 'locations-recommenders-get-config', 'locations-recommenders-recommendations-get', 'locations-recommenders-recommendations-list', 'locations-recommenders-recommendations-mark-claimed', 'locations-recommenders-recommendations-mark-failed' and 'locations-recommenders-recommendations-mark-succeeded'", vec![
            ("locations-insight-types-config",
                    Some(r##"Updates an InsightTypeConfig change. This will create a new revision of the config."##),
                    "Details at http://byron.github.io/google-apis-rs/google_recommender1_beta1_cli/organizations_locations-insight-types-config",
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Name of insight type config. Eg, projects/[PROJECT_NUMBER]/locations/[LOCATION]/insightTypes/[INSIGHT_TYPE_ID]/config"##),
                     Some(true),
                     Some(false)),
            ...
            ("locations-recommenders-config",
                    Some(r##"Updates a Recommender Config. This will create a new revision of the config."##),
                    "Details at http://byron.github.io/google-apis-rs/google_recommender1_beta1_cli/organizations_locations-recommenders-config",
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Name of recommender config. Eg, projects/[PROJECT_NUMBER]/locations/[LOCATION]/recommenders/[RECOMMENDER_ID]/config"##),
                     Some(true),
                     Some(false)),
