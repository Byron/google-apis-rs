...
use google_ids1::{api, Error, oauth2};
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["create-time", "description", "endpoint-forwarding-rule", "endpoint-ip", "labels", "name", "network", "severity", "state", "traffic-logs", "update-time"]);
...
async fn main() {
    ...
    let arg_data = [
        ("projects", "methods: 'locations-endpoints-create', 'locations-endpoints-delete', 'locations-endpoints-get', 'locations-endpoints-get-iam-policy', 'locations-endpoints-list', 'locations-endpoints-set-iam-policy', 'locations-endpoints-test-iam-permissions', 'locations-get', 'locations-list', 'locations-operations-cancel', 'locations-operations-delete', 'locations-operations-get' and 'locations-operations-list'", vec![
