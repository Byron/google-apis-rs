...
use google_documentai1_beta2::{api, yup_oauth2, Error};
...
{
    async fn _projects_documents_batch_process(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut request: api::GoogleCloudDocumentaiV1beta2BatchProcessDocumentsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .documents_batch_process(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_documents_process(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "entity-extraction-params.enabled" => Some((
                ...
                "entity-extraction-params.model-version" => Some((
                ...
                "form-extraction-params.enabled" => Some((
                ...
                "form-extraction-params.model-version" => Some((
                ...
                "ocr-params.language-hints" => Some((
                ...
                "output-config.pages-per-shard" => Some((
                ...
                "table-extraction-params.enabled" => Some((
                ...
                "table-extraction-params.header-hints" => Some((
                ...
                "table-extraction-params.model-version" => Some((
        ...
        let mut request: api::GoogleCloudDocumentaiV1beta2ProcessDocumentRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .documents_process(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_documents_batch_process(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_documents_batch_process(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_documents_process(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_documents_process(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
        ...
        match self.opt.subcommand() {
            ("projects", Some(opt)) => match opt.subcommand() {
                ("documents-batch-process", Some(opt)) => {
                    call_result = self
                        ._projects_documents_batch_process(opt, dry_run, &mut err)
                        .await;
                }
                ("documents-process", Some(opt)) => {
                    call_result = self
                        ._projects_documents_process(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-documents-batch-process", Some(opt)) => {
                    call_result = self
                        ._projects_locations_documents_batch_process(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-documents-process", Some(opt)) => {
                    call_result = self
                        ._projects_locations_documents_process(opt, dry_run, &mut err)
                        .await;
                }
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "documentai1-beta2-secret.json",
