...
use google_realtimebidding1::{api, Error};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::RealTimeBidding<hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>
    >,
    gp: Vec<&'static str>,
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           ...
                                                                           v.extend(["view", "page-token", "page-size", "filter"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
...
async fn main() {
    ...
    let arg_data = [
        ("bidders", "methods: 'creatives-list', 'creatives-watch', 'endpoints-get', 'endpoints-list', 'get', 'list', 'pretargeting-configs-activate', 'pretargeting-configs-add-targeted-apps', 'pretargeting-configs-add-targeted-publishers', 'pretargeting-configs-add-targeted-sites', 'pretargeting-configs-create', 'pretargeting-configs-delete', 'pretargeting-configs-get', 'pretargeting-configs-list', 'pretargeting-configs-patch', 'pretargeting-configs-remove-targeted-apps', 'pretargeting-configs-remove-targeted-publishers', 'pretargeting-configs-remove-targeted-sites' and 'pretargeting-configs-suspend'", vec![
            ("creatives-list",
