...
use google_cloudlatencytest2::{api, Error};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::Cloudlatencytest<hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>
    >,
    gp: Vec<&'static str>,
...
impl<'n> Engine<'n> {
    async fn _statscollection_updateaggregatedstats(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut request: api::AggregatedStats = json::value::from_value(object).unwrap();
    ...
    async fn _statscollection_updatestats(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut request: api::Stats = json::value::from_value(object).unwrap();
            ...
            ("statscollection", Some(opt)) => {
                match opt.subcommand() {
                    ("updateaggregatedstats", Some(opt)) => {
                        call_result = self._statscollection_updateaggregatedstats(opt, dry_run, &mut err).await;
                    },
                    ("updatestats", Some(opt)) => {
                        call_result = self._statscollection_updatestats(opt, dry_run, &mut err).await;
                    },
                    _ => {
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "cloudlatencytest2-secret.json",
        ...
        ).persist_tokens_to_disk(format!("{}/cloudlatencytest2", config_dir)).build().await.unwrap();
