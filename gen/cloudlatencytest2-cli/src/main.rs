...
use google_cloudlatencytest2::{api, yup_oauth2, Error};
...
{
    async fn _statscollection_updateaggregatedstats(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut request: api::AggregatedStats = serde_json::value::from_value(object).unwrap();
    ...
    async fn _statscollection_updatestats(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::Stats = serde_json::value::from_value(object).unwrap();
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("statscollection", Some(opt)) => match opt.subcommand() {
                ("updateaggregatedstats", Some(opt)) => {
                    call_result = self
                        ._statscollection_updateaggregatedstats(opt, dry_run, &mut err)
                        .await;
                }
                ("updatestats", Some(opt)) => {
                    call_result = self
                        ._statscollection_updatestats(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("statscollection".to_string()));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "cloudlatencytest2-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/cloudlatencytest2", config_dir))
        .build()
        .await
        .unwrap();
