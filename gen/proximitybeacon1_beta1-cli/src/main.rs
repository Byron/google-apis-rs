...
use google_proximitybeacon1_beta1::{api, yup_oauth2, Error};
...
{
    async fn _beaconinfo_getforobserved(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut request: api::GetInfoForObservedBeaconsRequest =
    ...
    async fn _beacons_activate(
        ...
        let mut call = self
            .hub
            .beacons()
            .activate(opt.value_of("beacon-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _beacons_attachments_batch_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .beacons()
            .attachments_batch_delete(opt.value_of("beacon-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _beacons_attachments_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::BeaconAttachment = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .beacons()
            .attachments_create(request, opt.value_of("beacon-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _beacons_attachments_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .beacons()
            .attachments_delete(opt.value_of("attachment-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _beacons_attachments_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .beacons()
            .attachments_list(opt.value_of("beacon-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _beacons_deactivate(
        ...
        let mut call = self
            .hub
            .beacons()
            .deactivate(opt.value_of("beacon-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _beacons_decommission(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .beacons()
            .decommission(opt.value_of("beacon-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _beacons_delete(
        ...
        let mut call = self
            .hub
            .beacons()
            .delete(opt.value_of("beacon-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _beacons_diagnostics_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .beacons()
            .diagnostics_list(opt.value_of("beacon-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["alert-filter", "page-size", "page-token", "project-id"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
    ...
    async fn _beacons_get(
        ...
        let mut call = self
            .hub
            .beacons()
            .get(opt.value_of("beacon-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["page-size", "page-token", "project-id", "q"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
    ...
    async fn _beacons_register(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "ephemeral-id-registration.beacon-ecdh-public-key" => Some((
                ...
                "ephemeral-id-registration.beacon-identity-key" => Some((
                    "ephemeralIdRegistration.beaconIdentityKey",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "ephemeral-id-registration.initial-clock-value" => Some((
                    "ephemeralIdRegistration.initialClockValue",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "ephemeral-id-registration.initial-eid" => Some((
                    "ephemeralIdRegistration.initialEid",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "ephemeral-id-registration.rotation-period-exponent" => Some((
                    "ephemeralIdRegistration.rotationPeriodExponent",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "ephemeral-id-registration.service-ecdh-public-key" => Some((
                    "ephemeralIdRegistration.serviceEcdhPublicKey",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "expected-stability" => Some((
                    "expectedStability",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
        ...
        let mut request: api::Beacon = serde_json::value::from_value(object).unwrap();
    ...
    async fn _beacons_update(
        ...
        let mut call = self
            .hub
            .beacons()
            .update(request, opt.value_of("beacon-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _methods_get_eidparams(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _namespaces_update(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["namespace-name", "serving-visibility"],
                    );
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
        ...
        let mut call = self
            .hub
            .namespaces()
            .update(request, opt.value_of("namespace-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("beaconinfo", Some(opt)) => match opt.subcommand() {
                ("getforobserved", Some(opt)) => {
                    call_result = self
                        ._beaconinfo_getforobserved(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("beaconinfo".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("beacons", Some(opt)) => match opt.subcommand() {
                ("activate", Some(opt)) => {
                    call_result = self._beacons_activate(opt, dry_run, &mut err).await;
                }
                ("attachments-batch-delete", Some(opt)) => {
                    call_result = self
                        ._beacons_attachments_batch_delete(opt, dry_run, &mut err)
                        .await;
                }
                ("attachments-create", Some(opt)) => {
                    call_result = self
                        ._beacons_attachments_create(opt, dry_run, &mut err)
                        .await;
                }
                ("attachments-delete", Some(opt)) => {
                    call_result = self
                        ._beacons_attachments_delete(opt, dry_run, &mut err)
                        .await;
                }
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("beacons".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "proximitybeacon1-beta1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/proximitybeacon1-beta1", config_dir))
        .build()
        .await
        .unwrap();
