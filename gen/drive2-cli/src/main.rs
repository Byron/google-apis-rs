...
impl Engine {
    ...
    fn _children_insert(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::ChildReference = Default::default();
    ...
    fn _comments_insert(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::Comment = Default::default();
        ...
        for kvarg in self.opt.arg_kv.iter() {
            ...
            match &field_name.to_string()[..] {
                ...
                "author.picture.url" => {
                        ...
                        request.author.as_mut().unwrap().picture.url = value.unwrap_or("").to_string();
                    },
                "author.kind" => {
                        request_author_init(&mut request);
                        request.author.as_mut().unwrap().kind = value.unwrap_or("").to_string();
                    },
                "author.display-name" => {
                        request_author_init(&mut request);
                ...
                "author.permission-id" => {
                        request_author_init(&mut request);
                        request.author.as_mut().unwrap().permission_id = value.unwrap_or("").to_string();
                    },
                "author.is-authenticated-user" => {
                        request_author_init(&mut request);
                        request.author.as_mut().unwrap().is_authenticated_user = arg_from_str(value.unwrap_or("false"), err, "author.is-authenticated-user", "boolean");
                    },
                "author.email-address" => {
                        request_author_init(&mut request);
                        request.author.as_mut().unwrap().email_address = value.unwrap_or("").to_string();
                    },
                "deleted" => {
                        request_author_init(&mut request);
                ...
                "context.type" => {
                        ...
                        request.context.as_mut().unwrap().type_ = value.unwrap_or("").to_string();
                    },
                "context.value" => {
                        request_context_init(&mut request);
                        request.context.as_mut().unwrap().value = value.unwrap_or("").to_string();
                    },
                "created-date" => {
                        request_context_init(&mut request);
    ...
    fn _files_copy(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::File = Default::default();
        ...
        for kvarg in self.opt.arg_kv.iter() {
            ...
            match &field_name.to_string()[..] {
                ...
                "labels.restricted" => {
                        ...
                        request.labels.as_mut().unwrap().restricted = arg_from_str(value.unwrap_or("false"), err, "labels.restricted", "boolean");
                    },
                "labels.hidden" => {
                        request_labels_init(&mut request);
                        request.labels.as_mut().unwrap().hidden = arg_from_str(value.unwrap_or("false"), err, "labels.hidden", "boolean");
                    },
                "labels.viewed" => {
                        request_labels_init(&mut request);
                        request.labels.as_mut().unwrap().viewed = arg_from_str(value.unwrap_or("false"), err, "labels.viewed", "boolean");
                    },
                "labels.starred" => {
                        request_labels_init(&mut request);
                        request.labels.as_mut().unwrap().starred = arg_from_str(value.unwrap_or("false"), err, "labels.starred", "boolean");
                    },
                "labels.trashed" => {
                        request_labels_init(&mut request);
                        request.labels.as_mut().unwrap().trashed = arg_from_str(value.unwrap_or("false"), err, "labels.trashed", "boolean");
                    },
                "indexable-text.text" => {
                        request_indexable_text_init(&mut request);
                        request.indexable_text.as_mut().unwrap().text = value.unwrap_or("").to_string();
                    },
                "explicitly-trashed" => {
                        request_indexable_text_init(&mut request);
                ...
                "sharing-user.picture.url" => {
                        request_sharing_user_init(&mut request);
                        request.sharing_user.as_mut().unwrap().picture.url = value.unwrap_or("").to_string();
                    },
                "sharing-user.kind" => {
                        request_sharing_user_init(&mut request);
                        request.sharing_user.as_mut().unwrap().kind = value.unwrap_or("").to_string();
                    },
                "sharing-user.display-name" => {
                        request_sharing_user_init(&mut request);
                        request.sharing_user.as_mut().unwrap().display_name = value.unwrap_or("").to_string();
                    },
                "sharing-user.permission-id" => {
                        request_sharing_user_init(&mut request);
                        request.sharing_user.as_mut().unwrap().permission_id = value.unwrap_or("").to_string();
                    },
                "sharing-user.is-authenticated-user" => {
                        request_sharing_user_init(&mut request);
                        request.sharing_user.as_mut().unwrap().is_authenticated_user = arg_from_str(value.unwrap_or("false"), err, "sharing-user.is-authenticated-user", "boolean");
                    },
                "sharing-user.email-address" => {
                        request_sharing_user_init(&mut request);
                        request.sharing_user.as_mut().unwrap().email_address = value.unwrap_or("").to_string();
                    },
                "video-media-metadata.width" => {
                        request_video_media_metadata_init(&mut request);
                        request.video_media_metadata.as_mut().unwrap().width = arg_from_str(value.unwrap_or("-0"), err, "video-media-metadata.width", "integer");
                    },
                "video-media-metadata.duration-millis" => {
                        request_video_media_metadata_init(&mut request);
                        request.video_media_metadata.as_mut().unwrap().duration_millis = value.unwrap_or("").to_string();
                    },
                "video-media-metadata.height" => {
                        request_video_media_metadata_init(&mut request);
                        request.video_media_metadata.as_mut().unwrap().height = arg_from_str(value.unwrap_or("-0"), err, "video-media-metadata.height", "integer");
                    },
                "last-modifying-user.picture.url" => {
                        request_last_modifying_user_init(&mut request);
                        request.last_modifying_user.as_mut().unwrap().picture.url = value.unwrap_or("").to_string();
                    },
                "last-modifying-user.kind" => {
                        request_last_modifying_user_init(&mut request);
                        request.last_modifying_user.as_mut().unwrap().kind = value.unwrap_or("").to_string();
                    },
                "last-modifying-user.display-name" => {
                        request_last_modifying_user_init(&mut request);
                        request.last_modifying_user.as_mut().unwrap().display_name = value.unwrap_or("").to_string();
                    },
                "last-modifying-user.permission-id" => {
                        request_last_modifying_user_init(&mut request);
                        request.last_modifying_user.as_mut().unwrap().permission_id = value.unwrap_or("").to_string();
                    },
                "last-modifying-user.is-authenticated-user" => {
                        request_last_modifying_user_init(&mut request);
                        request.last_modifying_user.as_mut().unwrap().is_authenticated_user = arg_from_str(value.unwrap_or("false"), err, "last-modifying-user.is-authenticated-user", "boolean");
                    },
                "last-modifying-user.email-address" => {
                        request_last_modifying_user_init(&mut request);
                        request.last_modifying_user.as_mut().unwrap().email_address = value.unwrap_or("").to_string();
                    },
                "copyable" => {
                        request_last_modifying_user_init(&mut request);
                ...
                "export-links" => {
                        ...
                        request.export_links.as_mut().unwrap().push(value.unwrap_or("").to_string());
                    },
                "shared" => {
                        request_last_modifying_user_init(&mut request);
                ...
                "thumbnail.mime-type" => {
                        ...
                        request.thumbnail.as_mut().unwrap().mime_type = value.unwrap_or("").to_string();
                    },
                "thumbnail.image" => {
                        request_thumbnail_init(&mut request);
                        request.thumbnail.as_mut().unwrap().image = value.unwrap_or("").to_string();
                    },
                "open-with-links" => {
                        request_thumbnail_init(&mut request);
                        ...
                        request.open_with_links.as_mut().unwrap().push(value.unwrap_or("").to_string());
                    },
                "image-media-metadata.exposure-bias" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().exposure_bias = arg_from_str(value.unwrap_or("0.0"), err, "image-media-metadata.exposure-bias", "number");
                    },
                "image-media-metadata.exposure-time" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().exposure_time = arg_from_str(value.unwrap_or("0.0"), err, "image-media-metadata.exposure-time", "number");
                    },
                "image-media-metadata.max-aperture-value" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().max_aperture_value = arg_from_str(value.unwrap_or("0.0"), err, "image-media-metadata.max-aperture-value", "number");
                    },
                "image-media-metadata.width" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().width = arg_from_str(value.unwrap_or("-0"), err, "image-media-metadata.width", "integer");
                    },
                "image-media-metadata.focal-length" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().focal_length = arg_from_str(value.unwrap_or("0.0"), err, "image-media-metadata.focal-length", "number");
                    },
                "image-media-metadata.camera-make" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().camera_make = value.unwrap_or("").to_string();
                    },
                "image-media-metadata.exposure-mode" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().exposure_mode = value.unwrap_or("").to_string();
                    },
                "image-media-metadata.color-space" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().color_space = value.unwrap_or("").to_string();
                    },
                "image-media-metadata.location.latitude" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().location.latitude = arg_from_str(value.unwrap_or("0.0"), err, "image-media-metadata.location.latitude", "number");
                    },
                "image-media-metadata.location.altitude" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().location.altitude = arg_from_str(value.unwrap_or("0.0"), err, "image-media-metadata.location.altitude", "number");
                    },
                "image-media-metadata.location.longitude" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().location.longitude = arg_from_str(value.unwrap_or("0.0"), err, "image-media-metadata.location.longitude", "number");
                    },
                "image-media-metadata.subject-distance" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().subject_distance = arg_from_str(value.unwrap_or("-0"), err, "image-media-metadata.subject-distance", "integer");
                    },
                "image-media-metadata.height" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().height = arg_from_str(value.unwrap_or("-0"), err, "image-media-metadata.height", "integer");
                    },
                "image-media-metadata.lens" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().lens = value.unwrap_or("").to_string();
                    },
                "image-media-metadata.date" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().date = value.unwrap_or("").to_string();
                    },
                "image-media-metadata.iso-speed" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().iso_speed = arg_from_str(value.unwrap_or("-0"), err, "image-media-metadata.iso-speed", "integer");
                    },
                "image-media-metadata.metering-mode" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().metering_mode = value.unwrap_or("").to_string();
                    },
                "image-media-metadata.flash-used" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().flash_used = arg_from_str(value.unwrap_or("false"), err, "image-media-metadata.flash-used", "boolean");
                    },
                "image-media-metadata.aperture" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().aperture = arg_from_str(value.unwrap_or("0.0"), err, "image-media-metadata.aperture", "number");
                    },
                "image-media-metadata.rotation" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().rotation = arg_from_str(value.unwrap_or("-0"), err, "image-media-metadata.rotation", "integer");
                    },
                "image-media-metadata.sensor" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().sensor = value.unwrap_or("").to_string();
                    },
                "image-media-metadata.white-balance" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().white_balance = value.unwrap_or("").to_string();
                    },
                "image-media-metadata.camera-model" => {
                        request_image_media_metadata_init(&mut request);
                        request.image_media_metadata.as_mut().unwrap().camera_model = value.unwrap_or("").to_string();
                    },
                "description" => {
                        request_image_media_metadata_init(&mut request);
    ...
    fn _files_get(&self, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for parg in self.opt.arg_v.iter() {
            ...
            match key {
                ...
                "alt" => {
                    if value.unwrap_or("") == "media" {
                        download_mode = true;
                    }
                    call = call.alt(value.unwrap_or(""));
                },
                "acknowledge-abuse" => {
    ...
    fn _parents_insert(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::ParentReference = Default::default();
    ...
    fn _permissions_insert(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::Permission = Default::default();
    ...
    fn _properties_insert(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::Property = Default::default();
    ...
    fn _replies_insert(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::CommentReply = Default::default();
    ...
    fn _revisions_patch(&self, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Option<api::Error> {
            let mut request: api::Revision = Default::default();
    ...
    fn new(opt: Options) -> Result<Engine, InvalidOptionsError> {
        ...
        let engine = Engine {
            opt: opt,
            hub: api::Drive::new(hyper::Client::new(), auth),
        };
