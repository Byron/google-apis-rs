...
//! This documentation was generated from *fitness* crate version *0.1.0+20150222*, where *20150222* is the exact revision of the *fitness:v1* schema built by the [mako](http://www.makotemplates.org/) code generator *v0.1.0*.
//! 
...
impl<'a, C, NC, A> UserDataSourceDatasetGetCall<'a, C, NC, A> where NC: hyper::net::NetworkConnector, C: BorrowMut<hyper::Client<NC>>, A: oauth2::GetToken {
    ...
    pub fn doit(mut self) -> Result<(hyper::client::Response, Dataset)> {
        ...
        if self._scopes.len() == 0 {
            self._scopes.insert(Scope::ActivityRead.as_slice().to_string(), ());
        }
        ...
        }
        {
            ...
            for param_name in ["userId", "dataSourceId", "datasetId"].iter() {
...
impl<'a, C, NC, A> UserSessionDeleteCall<'a, C, NC, A> where NC: hyper::net::NetworkConnector, C: BorrowMut<hyper::Client<NC>>, A: oauth2::GetToken {
    ...
    pub fn doit(mut self) -> Result<hyper::client::Response> {
        ...
        }
        {
            ...
            for param_name in ["userId", "sessionId"].iter() {
                for (index, &(ref name, _)) in params.iter().rev().enumerate() {
                    if name == param_name {
                        indices_for_removal.push(params.len() - index - 1);
                        break;
