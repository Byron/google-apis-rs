...
//! This documentation was generated from *Apigee* crate version *7.0.0+20251216*, where *20251216* is the exact revision of the *apigee:v1* schema built by the [mako](http://www.makotemplates.org/) code generator *v7.0.0*.
//!
...
//!
//! ```ignore
//! let r = hub.organizations().apim_service_extensions_create(...).doit().await
//! let r = hub.organizations().apim_service_extensions_delete(...).doit().await
//! let r = hub.organizations().apim_service_extensions_patch(...).doit().await
//! let r = hub.organizations().dns_zones_create(...).doit().await
//! let r = hub.organizations().dns_zones_delete(...).doit().await
...
//! let r = hub.organizations().update_control_plane_access(...).doit().await
...
//! use apigee1::api::GoogleCloudApigeeV1ApimServiceExtension;
...
//! let mut req = GoogleCloudApigeeV1ApimServiceExtension::default();
//!
//! // You can configure optional parameters by calling the respective setters at will, and
//! // execute the final call using `doit()`.
//! // Values shown here are possibly random and not representative !
//! let result = hub.organizations().apim_service_extensions_patch(req, "name")
