...
impl<'n> Engine<'n> {
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["ages", "audience", "cost", "country", "country-subdivision", "currency-code", "customer-data", "description", "gender", "languages", "nanos", "owners", "population-source", "state", "survey-url-id", "title", "wanted-response-count"]);
                        err.issues.push(CLIError::Field(FieldError::Unknown(temp_cursor.to_string(), suggestion, value.map(|v| v.to_string()))));
                        None
                    }
                };
    ...
    fn _surveys_start(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.surveys().start(opt.value_of("resource-id").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
...
fn main() {
    ...
    let arg_data = [
        ("results", "methods: 'get'", vec![
            ("get",
                    Some(r##"Retrieves any survey results that have been produced so far. Results are formatted as an Excel file."##),
        ...
        ("surveys", "methods: 'get', 'insert', 'list', 'start', 'stop' and 'update'", vec![
            ("get",
