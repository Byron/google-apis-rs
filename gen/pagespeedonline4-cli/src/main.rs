...
use google_pagespeedonline4::{api, Error};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::Pagespeedonline<hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>
    >,
    gp: Vec<&'static str>,
...
impl<'n> Engine<'n> {
    async fn _pagespeedapi_runpagespeed(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                ...
                "snapshots" => {
                    call = call.snapshots(arg_from_str(value.unwrap_or("false"), err, "snapshots", "boolean"));
                },
                "screenshot" => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           ...
                                                                           v.extend(["rule", "utm-source", "strategy", "locale", "utm-campaign", "screenshot", "filter-third-party-resources", "snapshots"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
    ...
    async fn _doit(&self, dry_run: bool) -> Result<Result<(), DoitError>, Option<InvalidOptionsError>> {
        ...
        match self.opt.subcommand() {
            ("pagespeedapi", Some(opt)) => {
                match opt.subcommand() {
                    ("runpagespeed", Some(opt)) => {
                        call_result = self._pagespeedapi_runpagespeed(opt, dry_run, &mut err).await;
                    },
                    _ => {
                        err.issues.push(CLIError::MissingMethodError("pagespeedapi".to_string()));
                        writeln!(io::stderr(), "{}\n", opt.usage()).ok();
                    }
                }
            },
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "pagespeedonline4-secret.json",
    ...
    let arg_data = [
        ("pagespeedapi", "methods: 'runpagespeed'", vec![
            ("runpagespeed",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_pagespeedonline4_cli/pagespeedapi_runpagespeed",
                  vec![
                    (Some(r##"url"##),
                     None,
