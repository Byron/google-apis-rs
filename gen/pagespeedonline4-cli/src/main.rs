...
use google_pagespeedonline4::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
            ...
            .iter()
        {
            ...
            match key {
                ...
                "snapshots" => {
                    call = call.snapshots(
                        value
                            .map(|v| arg_from_str(v, err, "snapshots", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "screenshot" => {
                    call = call.screenshot(
                        value
                            .map(|v| arg_from_str(v, err, "screenshot", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "rule" => {
                ...
                "filter-third-party-resources" => {
                    call = call.filter_third_party_resources(
                        value
                            .map(|v| {
                                arg_from_str(v, err, "filter-third-party-resources", "boolean")
                            })
                            .unwrap_or(false),
                    );
                }
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "pagespeedonline4-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/pagespeedonline4", config_dir))
        .build()
        .await
        .unwrap();
