...
use google_pagespeedonline4::{api, Error, oauth2, client::chrono, FieldMask};
    ...
    async fn _pagespeedapi_runpagespeed(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                ...
                "snapshots" => {
                    call = call.snapshots(        value.map(|v| arg_from_str(v, err, "snapshots", "boolean")).unwrap_or(false));
                },
                "screenshot" => {
