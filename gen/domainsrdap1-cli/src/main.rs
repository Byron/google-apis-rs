...
use google_domainsrdap1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _autnum_get(
        ...
        let mut call = self
            .hub
            .autnum()
            .get(opt.value_of("autnum-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _domain_get(
        ...
        let mut call = self
            .hub
            .domain()
            .get(opt.value_of("domain-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _entity_get(
        ...
        let mut call = self
            .hub
            .entity()
            .get(opt.value_of("entity-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _ip_get(
        ...
        let mut call = self.hub.ip().get(
            opt.value_of("ip-id").unwrap_or(""),
            opt.value_of("ip-id1").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
    ...
    async fn _methods_get_entities(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _methods_get_nameservers(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _nameserver_get(
        ...
        let mut call = self
            .hub
            .nameserver()
            .get(opt.value_of("nameserver-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("autnum", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._autnum_get(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("autnum".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("domain", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._domain_get(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("domain".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("entity", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._entity_get(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("entity".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("ip", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._ip_get(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("ip".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ...
            ("nameserver", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._nameserver_get(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("nameserver".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "domainsrdap1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/domainsrdap1", config_dir))
        .build()
        .await
        .unwrap();
