...
use google_analyticsdata1_beta::{api, Error};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::AnalyticsData<hyper::Client<hyper_rustls::HttpsConnector<hyper::client::connect::HttpConnector>, hyper::body::Body>
    >,
    gp: Vec<&'static str>,
...
async fn main() {
    ...
    let arg_data = [
        ("properties", "methods: 'batch-run-pivot-reports', 'batch-run-reports', 'get-metadata', 'run-pivot-report', 'run-realtime-report' and 'run-report'", vec![
