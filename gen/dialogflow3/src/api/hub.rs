...
impl<'a, S> client::Hub for Dialogflow<S> {}

impl<'a, S> Dialogflow<S> {

    pub fn new<A: 'static + client::GetToken>(client: hyper::Client<S, hyper::body::Body>, auth: A) -> Dialogflow<S> {
        Dialogflow {
            client,
            auth: Box::new(auth),
