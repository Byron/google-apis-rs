...
extern crate mime;
extern crate strsim;
extern crate google_cloudasset1_beta1 as api;
...
impl<'n> Engine<'n> {
    ...
    fn _organizations_export_assets(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    fn _projects_batch_get_assets_history(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    fn _projects_export_assets(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    fn _doit(&self, dry_run: bool) -> Result<Result<(), DoitError>, Option<InvalidOptionsError>> {
        ...
        match self.opt.subcommand() {
            ("organizations", Some(opt)) => {
                match opt.subcommand() {
                    ("batch-get-assets-history", Some(opt)) => {
                        call_result = self._organizations_batch_get_assets_history(opt, dry_run, &mut err);
                    },
                    ("export-assets", Some(opt)) => {
                        call_result = self._organizations_export_assets(opt, dry_run, &mut err);
                    },
            ...
            ("projects", Some(opt)) => {
                match opt.subcommand() {
                    ("batch-get-assets-history", Some(opt)) => {
                        call_result = self._projects_batch_get_assets_history(opt, dry_run, &mut err);
                    },
                    ("export-assets", Some(opt)) => {
                        call_result = self._projects_export_assets(opt, dry_run, &mut err);
                    },
                    ("operations-get", Some(opt)) => {
    ...
    fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match cmn::application_secret_from_directory(&config_dir, "cloudasset1-beta1-secret.json",
        ...
        let auth = Authenticator::new(  &secret, DefaultAuthenticatorDelegate,
                                        ...
                                        JsonTokenStorage {
                                          program_name: "cloudasset1-beta1",
