...
use google_playablelocations3::{api, yup_oauth2, Error};
...
{
    async fn _methods_log_impressions(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "client-info.application-id" => Some((
                ...
                "client-info.application-version" => Some((
                ...
                "client-info.language-code" => Some((
                ...
                "client-info.operating-system" => Some((
                ...
                "client-info.operating-system-build" => Some((
                    "clientInfo.operatingSystemBuild",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "client-info.platform" => Some((
                    "clientInfo.platform",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
        ...
        let mut request: api::GoogleMapsPlayablelocationsV3LogImpressionsRequest =
    ...
    async fn _methods_log_player_reports(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::GoogleMapsPlayablelocationsV3LogPlayerReportsRequest =
            serde_json::value::from_value(object).unwrap();
    ...
    async fn _methods_sample_playable_locations(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion =
                        FieldCursor::did_you_mean(key, &vec!["area-filter", "s2-cell-id"]);
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
                        value.map(|v| v.to_string()),
        ...
        let mut request: api::GoogleMapsPlayablelocationsV3SamplePlayableLocationsRequest =
            serde_json::value::from_value(object).unwrap();
        ...
        match self.opt.subcommand() {
            ("methods", Some(opt)) => match opt.subcommand() {
                ...
                ("log-player-reports", Some(opt)) => {
                    call_result = self
                        ._methods_log_player_reports(opt, dry_run, &mut err)
                        .await;
                }
                ("sample-playable-locations", Some(opt)) => {
                    call_result = self
                        ._methods_sample_playable_locations(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "playablelocations3-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/playablelocations3", config_dir))
        .build()
        .await
        .unwrap();
