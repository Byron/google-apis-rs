...
impl<'n> Engine<'n> {
    fn _activities_list(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.activities().list(opt.value_of("customer-id").unwrap_or(""), opt.value_of("application-id").unwrap_or(""));
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           ...
                                                                           v.extend(["actor-email", "actor-application-id", "actor-ip-address", "caller", "max-results", "event-name", "start-time", "end-time", "continuation-token"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
    ...
    let arg_data = [
        ("activities", "methods: 'list'", vec![
            ("list",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_audit1_cli/activities_list",
                  vec![
                    (Some(r##"customer-id"##),
                     None,
                     Some(r##"Represents the customer who is the owner of target object on which action was performed."##),
                     Some(true),
                     Some(false)),
        
                    (Some(r##"application-id"##),
                     None,
                     Some(r##"Application ID of the application on which the event was performed."##),
                     Some(true),
                     Some(false)),
