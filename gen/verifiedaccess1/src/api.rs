...
impl<C> common::Hub for Verifiedaccess<C> {}
...
impl common::ResponseResult for Challenge {}
...
impl common::RequestValue for Empty {}
...
impl common::Part for SignedData {}
...
impl common::RequestValue for VerifyChallengeResponseRequest {}
...
impl common::ResponseResult for VerifyChallengeResponseResult {}
...
impl<'a, C> common::MethodsBuilder for ChallengeMethods<'a, C> {}
...
impl<'a, C> common::CallBuilder for ChallengeCreateCall<'a, C> {}

impl<'a, C> ChallengeCreateCall<'a, C>
where
    C: common::Connector,
{
    /// Perform the operation you have build so far.
    pub async fn doit(mut self) -> common::Result<(common::Response, Challenge)> {
        use std::borrow::Cow;
    ...
    pub fn add_scope<St>(mut self, scope: St) -> ChallengeCreateCall<'a, C>
    where
        St: AsRef<str>,
    {
    ...
    pub fn add_scopes<I, St>(mut self, scopes: I) -> ChallengeCreateCall<'a, C>
    where
    ...
    /// for details).
    pub fn clear_scopes(mut self) -> ChallengeCreateCall<'a, C> {
        self._scopes.clear();
        self
    }
}
...
impl<'a, C> common::CallBuilder for ChallengeVerifyCall<'a, C> {}

impl<'a, C> ChallengeVerifyCall<'a, C>
where
    C: common::Connector,
{
    /// Perform the operation you have build so far.
    pub async fn doit(
        mut self,
    ) -> common::Result<(common::Response, VerifyChallengeResponseResult)> {
        use std::borrow::Cow;
        use std::io::{Read, Seek};
    ...
    pub fn request(
        mut self,
        new_value: VerifyChallengeResponseRequest,
    ) -> ChallengeVerifyCall<'a, C> {
        self._request = new_value;
        self
    }
    /// The delegate implementation is consulted whenever there is an intermediate result, or if something goes wrong
    ...
    /// encompass more rights than others. For example, for listing resources, a *read-only* scope will be
    /// sufficient, a read-write scope will do as well.
    pub fn add_scope<St>(mut self, scope: St) -> ChallengeVerifyCall<'a, C>
    where
        St: AsRef<str>,
    {
        self._scopes.insert(String::from(scope.as_ref()));
    ...
    ///
    /// See [`Self::add_scope()`] for details.
    pub fn add_scopes<I, St>(mut self, scopes: I) -> ChallengeVerifyCall<'a, C>
    where
        I: IntoIterator<Item = St>,
        St: AsRef<str>,
    {
    ...
    /// In this case, you have to specify your API-key using the `key` parameter (see [`Self::param()`]
    /// for details).
    pub fn clear_scopes(mut self) -> ChallengeVerifyCall<'a, C> {
        self._scopes.clear();
        self
    }
}
