...
//! The original source code is [on github](https://github.com/Byron/google-apis-rs/tree/master/gen/verifiedaccess1).
//! # Features
//! 
//! Handle the following *Resources* with ease from the central [hub](struct.Verifiedaccess.html) ... 
//! 
//! * [challenge](struct.Challenge.html)
//!  * [*create*](struct.ChallengeCreateCall.html) and [*verify*](struct.ChallengeVerifyCall.html)
//! 
//! 
//! 
//! 
...
//! 
//! * **[Hub](struct.Verifiedaccess.html)**
...
impl<'a, C, A> Hub for Verifiedaccess<C, A> {}

impl<'a, C, A> Verifiedaccess<C, A>
    where  C: BorrowMut<hyper::Client>, A: oauth2::GetToken {

    pub fn new(client: C, authenticator: A) -> Verifiedaccess<C, A> {
        Verifiedaccess {
    ...
    pub fn challenge(&'a self) -> ChallengeMethods<'a, C, A> {
...
/// 
/// * [create challenge](struct.ChallengeCreateCall.html) (response)
/// 
...
/// The list links the activity name, along with information about where it is used (one of *request* and *response*).
/// 
/// * [verify challenge](struct.ChallengeVerifyCall.html) (request)
/// 
#[derive(Default, Clone, Debug, Serialize, Deserialize)]
...
/// The list links the activity name, along with information about where it is used (one of *request* and *response*).
/// 
/// * [create challenge](struct.ChallengeCreateCall.html) (request)
#[derive(Default, Clone, Debug, Serialize, Deserialize)]
pub struct Empty { _never_set: Option<bool> }

impl RequestValue for Empty {}
...
/// The list links the activity name, along with information about where it is used (one of *request* and *response*).
/// 
/// * [verify challenge](struct.ChallengeVerifyCall.html) (response)
/// 
#[derive(Default, Clone, Debug, Serialize, Deserialize)]
...
impl ResponseResult for VerifyChallengeResponseResult {}
...
/// # }
/// ```
pub struct ChallengeMethods<'a, C, A>
    where C: 'a, A: 'a {

    hub: &'a Verifiedaccess<C, A>,
}

impl<'a, C, A> MethodsBuilder for ChallengeMethods<'a, C, A> {}

impl<'a, C, A> ChallengeMethods<'a, C, A> {
    ...
    pub fn create(&self, request: Empty) -> ChallengeCreateCall<'a, C, A> {
        ChallengeCreateCall {
            hub: self.hub,
            _request: request,
    ...
    ///
    /// * `request` - No description provided.
    pub fn verify(&self, request: VerifyChallengeResponseRequest) -> ChallengeVerifyCall<'a, C, A> {
        ChallengeVerifyCall {
            hub: self.hub,
            _request: request,
            _delegate: Default::default(),
...
/// # }
/// ```
pub struct ChallengeCreateCall<'a, C, A>
    where C: 'a, A: 'a {
...
impl<'a, C, A> CallBuilder for ChallengeCreateCall<'a, C, A> {}

impl<'a, C, A> ChallengeCreateCall<'a, C, A> where C: BorrowMut<hyper::Client>, A: oauth2::GetToken {


    /// Perform the operation you have build so far.
    pub fn doit(mut self) -> Result<(hyper::client::Response, Challenge)> {
        ...
        dlg.begin(MethodInfo { id: "verifiedaccess.challenge.create",
    ...
    pub fn request(mut self, new_value: Empty) -> ChallengeCreateCall<'a, C, A> {
    ...
    pub fn delegate(mut self, new_value: &'a mut dyn Delegate) -> ChallengeCreateCall<'a, C, A> {
    ...
    pub fn param<T>(mut self, name: T, value: T) -> ChallengeCreateCall<'a, C, A>
                                                        where T: AsRef<str> {
    ...
    pub fn add_scope<T, S>(mut self, scope: T) -> ChallengeCreateCall<'a, C, A>
...
/// # }
/// ```
pub struct ChallengeVerifyCall<'a, C, A>
    where C: 'a, A: 'a {
...
impl<'a, C, A> CallBuilder for ChallengeVerifyCall<'a, C, A> {}

impl<'a, C, A> ChallengeVerifyCall<'a, C, A> where C: BorrowMut<hyper::Client>, A: oauth2::GetToken {


    /// Perform the operation you have build so far.
    pub fn doit(mut self) -> Result<(hyper::client::Response, VerifyChallengeResponseResult)> {
        use std::io::{Read, Seek};
        use hyper::header::{ContentType, ContentLength, Authorization, Bearer, UserAgent, Location};
        let mut dd = DefaultDelegate;
        let mut dlg: &mut dyn Delegate = match self._delegate {
        ...
        dlg.begin(MethodInfo { id: "verifiedaccess.challenge.verify",
                               http_method: hyper::method::Method::Post });
        let mut params: Vec<(&str, String)> = Vec::with_capacity(3 + self._additional_params.len());
        for &field in ["alt"].iter() {
            if self._additional_params.contains_key(field) {
    ...
    /// Even though the property as already been set when instantiating this call,
    /// we provide this method for API completeness.
    pub fn request(mut self, new_value: VerifyChallengeResponseRequest) -> ChallengeVerifyCall<'a, C, A> {
        self._request = new_value;
        self
    }
    /// The delegate implementation is consulted whenever there is an intermediate result, or if something goes wrong
    ...
    ///
    /// Sets the *delegate* property to the given value.
    pub fn delegate(mut self, new_value: &'a mut dyn Delegate) -> ChallengeVerifyCall<'a, C, A> {
        self._delegate = Some(new_value);
        self
    }
    ...
    /// * *alt* (query-string) - Data format for response.
    /// * *$.xgafv* (query-string) - V1 error format.
    pub fn param<T>(mut self, name: T, value: T) -> ChallengeVerifyCall<'a, C, A>
                                                        where T: AsRef<str> {
        self._additional_params.insert(name.as_ref().to_string(), value.as_ref().to_string());
        self
    }
    ...
    /// encompass more rights than others. For example, for listing resources, a *read-only* scope will be
    /// sufficient, a read-write scope will do as well.
    pub fn add_scope<T, S>(mut self, scope: T) -> ChallengeVerifyCall<'a, C, A>
                                                        where T: Into<Option<S>>,
                                                              S: AsRef<str> {
        match scope.into() {
          Some(scope) => self._scopes.insert(scope.as_ref().to_string(), ()),
