...
<%
    ...
    qualifier = 'pub '
    add_args = ''
    rtype = 'client::Result<client::Response>'
    response_schema = method_response(c, m)
    ...
    if response_schema:
        if not supports_download:
            reserved_params = ['alt']
        rtype = 'client::Result<(client::Response, %s)>' % (response_schema.id)
            ...
            % endif
                ...
                % if request_value:
                    % if not simple_media_param:
                        ...
                        .body(client::to_body(request_value_reader.get_ref().clone()))\
                    % else:
                        let mut body_reader_bytes = vec![];
                        body_reader.read_to_end(&mut body_reader_bytes).unwrap();
                        let request = req_builder
                            .header(CONTENT_TYPE, content_type.to_string())
                            .body(client::to_body(body_reader_bytes))\
...
;
            ...
            match req_result {
                ...
                Ok(res) => {
                    ...
                    let mut bytes = client::to_bytes(body).await.unwrap_or_default();
                    if !parts.status.is_success() {
                        let error = serde_json::from_str(&client::to_string(&bytes));
                        let response = client::to_response(parts, bytes);

                        if let client::Retry::After(d) = dlg.http_failure(&response, error.as_ref().ok()) {
                            sleep(d).await;
                            continue;
                        }
                        ...
                        return Err(match error {
                            Ok(value) => client::Error::BadRequest(value),
                            _ => client::Error::Failure(response),
                        });
                    }
                    % if resumable_media_param:
                    if protocol == ${PROTOCOL_TYPE_MAP[resumable_media_param.protocol]} {
                        ...
                        match upload_result {
                            ...
                            Some(Ok(response)) => {
                                ...
                                parts = parts_body.0;
                                bytes = client::to_bytes(parts_body.1).await.unwrap_or_default();
                                if !parts.status.is_success() {
                                    ## delegate was called in upload() already - don't tell him again
                                    dlg.store_upload_url(None);
                                    ${delegate_finish}(false);
                                    return Err(client::Error::Failure(client::to_response(parts, bytes)));
                                }
                            }
                        }
                    }
...
if enable_resource_parsing \
                    % endif
{
                        let encoded = client::to_string(&bytes);
                        match serde_json::from_str(&encoded) {
                            Ok(decoded) => (client::to_response(parts, bytes), decoded),
                            Err(error) => {
                                dlg.response_json_decode_error(&encoded, &error);
                                return Err(client::Error::JsonDecodeError(encoded.to_string(), error));
                            }
                        }
                    }\
                    % if supports_download:
else {
                    (client::to_response(parts, bytes), Default::default())
}\
                    % endif
;
                % else:
